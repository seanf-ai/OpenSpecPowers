...
export class ShowCommand {
  ...
  private async showDirect(itemName: string, params: { typeOverride?: ItemType; options: { json?: boolean; [k: string]: any } }): Promise<void> {
    ...
    if (!params.typeOverride && isChange && isSpec) {
      ...
      console.error('Pass --type change|spec, or use: openspecpowers change show / openspecpowers spec show');
      process.exitCode = 1;
      return;
    }
  ...
  private printNonInteractiveHint(): void {
    ...
    console.error('  openspecpowers show <item>');
    console.error('  openspecpowers change show');
    console.error('  openspecpowers spec show');
