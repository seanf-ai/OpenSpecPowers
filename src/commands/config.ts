...
const WORKFLOW_PROMPT_META: Record<string, WorkflowPromptMeta> = {
  ...
  onboard: {
    name: 'Onboard',
    description: 'Guided onboarding flow for OpenSpecPowers',
  },
};
...
function maybeWarnConfigDrift(
  ...
  const openspecpowersDir = path.join(projectDir, OPENSPEC_DIR_NAME);
  if (!fs.existsSync(openspecpowersDir)) {
    return;
  }
  if (!hasProjectConfigDrift(projectDir, state.workflows, state.delivery)) {
    return;
  }
  console.log(colorize('Warning: Global config is not applied to this project. Run `openspecpowers update` to sync.'));
}
...
export function registerConfigCommand(program: Command): void {
  const configCmd = program
    .command('config')
    .description('View and modify global OpenSpecPowers configuration')
  ...
  configCmd
    ...
    .action((key: string, value: string, options: { string?: boolean; allowUnknown?: boolean }) => {
      ...
      if (!keyValidation.valid && !allowUnknown) {
        ...
        console.error('Use "openspecpowers config list" to see available keys.');
  ...
  configCmd
    ...
    .action(async (options: { all?: boolean; yes?: boolean }) => {
      if (!options.all) {
        ...
        console.error('Usage: openspecpowers config reset --all [-y]');
        process.exitCode = 1;
        return;
      }
  ...
  configCmd
    ...
    .action(async (preset?: string) => {
      // Preset shortcut: `openspecpowers config profile core`
      if (preset === 'core') {
        const config = getGlobalConfig();
        config.profile = 'core';
        ...
        console.log('Config updated. Run `openspecpowers update` in your projects to apply.');
        return;
      }
      ...
      if (!process.stdout.isTTY) {
        console.error('Interactive mode required. Use `openspecpowers config profile core` or set config via environment/flags.');
        process.exitCode = 1;
        return;
      }
      ...
      try {
        ...
        // Check if inside an OpenSpecPowers project
        ...
        if (fs.existsSync(openspecpowersDir)) {
          ...
          if (applyNow) {
            try {
              execSync('npx openspecpowers update', { stdio: 'inherit', cwd: projectDir });
              console.log('Run `openspecpowers update` in your other projects to apply.');
            } catch {
              console.error('`openspecpowers update` failed. Please run it manually to apply the profile changes.');
              process.exitCode = 1;
            }
            return;
          }
