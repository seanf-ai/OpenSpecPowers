...
export class ValidateCommand {
  ...
  private printNonInteractiveHint(): void {
    ...
    console.error('  openspecpowers validate --all');
    console.error('  openspecpowers validate --changes');
    console.error('  openspecpowers validate --specs');
    console.error('  openspecpowers validate <item-name>');
  ...
  private async validateDirectItem(itemName: string, opts: { typeOverride?: ItemType; strict: boolean; json: boolean }): Promise<void> {
    ...
    if (!opts.typeOverride && isChange && isSpec) {
      ...
      console.error('Pass --type change|spec, or use: openspecpowers change validate / openspecpowers spec validate');
      process.exitCode = 1;
      return;
    }
  ...
  private async validateByType(type: ItemType, id: string, opts: { strict: boolean; json: boolean }): Promise<void> {
    ...
    if (type === 'change') {
      const changeDir = path.join(process.cwd(), 'openspecpowers', 'changes', id);
    ...
    const file = path.join(process.cwd(), 'openspecpowers', 'specs', id, 'spec.md');
    const start = Date.now();
  ...
  private printNextSteps(type: ItemType): void {
    ...
    if (type === 'change') {
      ...
      bullets.push('- Debug parsed deltas: openspecpowers change show <id> --json --deltas-only');
    } else {
