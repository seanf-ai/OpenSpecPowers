 ...
 *
 * @example
 * // Creates openspecpowers/changes/add-auth/ with default schema
 * const result = await createChange('/path/to/project', 'add-auth')
 * console.log(result.schema) // 'spec-driven' or value from config
 *
 * @example
 * // Creates openspecpowers/changes/add-auth/ with custom schema
...
export async function createChange(
  ...
  const changeDir = path.join(projectRoot, 'openspecpowers', 'changes', name);
