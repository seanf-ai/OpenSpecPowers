...
/**
 * Writes change metadata to .openspecpowers.yaml in the change directory.
 *
...
/**
 * Reads change metadata from .openspecpowers.yaml in the change directory.
 *
 * @param changeDir - The path to the change directory
 * @param projectRoot - Optional project root for project-local schema resolution
 ...
 * 2. Schema from .openspecpowers.yaml metadata (if exists)
 * 3. Schema from openspecpowers/config.yaml (if exists)
...
export function resolveSchemaForChange(
  ...
  // Derive project root from changeDir (changeDir is typically projectRoot/openspecpowers/changes/change-name)
