/**
 * Legacy cleanup module for detecting and removing OpenSpecPowers artifacts
 ...
 * These were config files created at project root with OpenSpecPowers markers.
 */
export const LEGACY_CONFIG_FILES = [
  ...
  'QODER.md',
  'IFLOW.md',
  'AGENTS.md', // root AGENTS.md (not openspecpowers/AGENTS.md)
  'QWEN.md',
] as const;
...
export const LEGACY_SLASH_COMMAND_PATHS: Record<string, LegacySlashCommandPattern> = {
  // Directory-based: .tooldir/commands/openspecpowers/ or .tooldir/commands/openspecpowers/*.md
  'claude': { type: 'directory', path: '.claude/commands/openspecpowers' },
  'codebuddy': { type: 'directory', path: '.codebuddy/commands/openspecpowers' },
  'qoder': { type: 'directory', path: '.qoder/commands/openspecpowers' },
  'crush': { type: 'directory', path: '.crush/commands/openspecpowers' },
  'gemini': { type: 'directory', path: '.gemini/commands/openspecpowers' },
  'costrict': { type: 'directory', path: '.cospec/openspecpowers/commands' },

  // File-based: individual openspecpowers-*.md files in a commands/workflows/prompts folder
  'cursor': { type: 'files', pattern: '.cursor/commands/openspecpowers-*.md' },
  'windsurf': { type: 'files', pattern: '.windsurf/workflows/openspecpowers-*.md' },
  'kilocode': { type: 'files', pattern: '.kilocode/workflows/openspecpowers-*.md' },
  'kiro': { type: 'files', pattern: '.kiro/prompts/openspecpowers-*.prompt.md' },
  'github-copilot': { type: 'files', pattern: '.github/prompts/openspecpowers-*.prompt.md' },
  'amazon-q': { type: 'files', pattern: '.amazonq/prompts/openspecpowers-*.md' },
  'cline': { type: 'files', pattern: '.clinerules/workflows/openspecpowers-*.md' },
  'roocode': { type: 'files', pattern: '.roo/commands/openspecpowers-*.md' },
  'auggie': { type: 'files', pattern: '.augment/commands/openspecpowers-*.md' },
  'factory': { type: 'files', pattern: '.factory/commands/openspecpowers-*.md' },
  'opencode': { type: 'files', pattern: ['.opencode/command/opsp-*.md', '.opencode/command/openspecpowers-*.md'] },
  'continue': { type: 'files', pattern: '.continue/prompts/openspecpowers-*.prompt' },
  'antigravity': { type: 'files', pattern: '.agent/workflows/openspecpowers-*.md' },
  'iflow': { type: 'files', pattern: '.iflow/commands/openspecpowers-*.md' },
  'qwen': { type: 'files', pattern: '.qwen/commands/openspecpowers-*.toml' },
  'codex': { type: 'files', pattern: '.codex/prompts/openspecpowers-*.md' },
};
...
export interface LegacyDetectionResult {
  /** Config files with OpenSpecPowers markers detected */
  configFiles: string[];
  ...
  /** Whether openspecpowers/AGENTS.md exists */
  hasOpenspecAgents: boolean;
  /** Whether openspecpowers/project.md exists (preserved, migration hint only) */
  hasProjectMd: boolean;
  /** Whether root AGENTS.md has OpenSpecPowers markers */
...
/**
 * Detects all legacy OpenSpecPowers artifacts in a project.
 *
...
/**
 * Detects legacy config files with OpenSpecPowers markers.
...
export async function detectLegacyConfigFiles(
  ...
  for (const fileName of LEGACY_CONFIG_FILES) {
    ...
    if (await FileSystemUtils.fileExists(filePath)) {
      ...
      if (hasOpenSpecPowersMarkers(content)) {
 ...
 *
 * @param projectPath - The root path of the project
 * @param pattern - Glob pattern like '.cursor/commands/openspecpowers-*.md'
...
async function findLegacySlashCommandFiles(
  ...
  try {
    ...
    // openspecpowers-*.md -> /^openspecpowers-.*\.md$/
    // openspecpowers-*.prompt.md -> /^openspecpowers-.*\.prompt\.md$/
    // openspecpowers-*.toml -> /^openspecpowers-.*\.toml$/
...
/**
 * Detects legacy OpenSpecPowers structure files (AGENTS.md and project.md).
 *
 * @param projectPath - The root path of the project
...
export async function detectLegacyStructureFiles(
  ...
  // Check for openspecpowers/AGENTS.md
  const openspecpowersAgentsPath = FileSystemUtils.joinPath(projectPath, 'openspecpowers', 'AGENTS.md');
  hasOpenspecAgents = await FileSystemUtils.fileExists(openspecpowersAgentsPath);

  // Check for openspecpowers/project.md (for migration messaging, not deleted)
  const projectMdPath = FileSystemUtils.joinPath(projectPath, 'openspecpowers', 'project.md');
  hasProjectMd = await FileSystemUtils.fileExists(projectMdPath);

  // Check for root AGENTS.md with OpenSpecPowers markers
  ...
  if (await FileSystemUtils.fileExists(rootAgentsPath)) {
    ...
    hasRootAgentsWithMarkers = hasOpenSpecPowersMarkers(content);
  }
...
/**
 * Checks if content contains OpenSpecPowers markers.
 *
 * @param content - File content to check
 * @returns True if both start and end markers are present
 */
export function hasOpenSpecPowersMarkers(content: string): boolean {
  return (
...
/**
 * Checks if file content is 100% OpenSpecPowers content (only markers and whitespace outside).
 *
 * @param content - File content to check
 * @returns True if content outside markers is only whitespace
 */
export function isOnlyOpenSpecPowersContent(content: string): boolean {
...
/**
 * Removes the OpenSpecPowers marker block from file content.
 ...
 *
 * @param content - File content with OpenSpecPowers markers
...
/**
 * Cleans up legacy OpenSpecPowers artifacts from a project.
 * Preserves openspecpowers/project.md (shows migration hint instead of deleting).
 *
 * @param projectPath - The root path of the project
...
export async function cleanupLegacyArtifacts(
  ...
  // Delete legacy slash command directories (these are 100% OpenSpecPowers-managed)
  ...
  // Delete legacy slash command files (these are 100% OpenSpecPowers-managed)
  ...
  // Delete openspecpowers/AGENTS.md (this is inside openspecpowers/, it's OpenSpecPowers-managed)
  if (detection.hasOpenspecAgents) {
    const agentsPath = FileSystemUtils.joinPath(projectPath, 'openspecpowers', 'AGENTS.md');
    if (await FileSystemUtils.fileExists(agentsPath)) {
      try {
        await fs.unlink(agentsPath);
        result.deletedFiles.push('openspecpowers/AGENTS.md');
      } catch (error: any) {
        result.errors.push(`Failed to delete openspecpowers/AGENTS.md: ${error.message}`);
      }
    }
  }

  // Handle root AGENTS.md with OpenSpecPowers markers - remove markers only, NEVER delete
...
export function formatCleanupSummary(result: CleanupResult): string {
  ...
  if (result.deletedFiles.length > 0 || result.deletedDirs.length > 0 || result.modifiedFiles.length > 0) {
    ...
    for (const dir of result.deletedDirs) {
      lines.push(`  ✓ Removed ${dir}/ (replaced by /opsp:*)`);
    }

    for (const file of result.modifiedFiles) {
      lines.push(`  ✓ Removed OpenSpecPowers markers from ${file}`);
    }
  }
 ...
 * Only includes OpenSpecPowers-managed files (slash commands, openspecpowers/AGENTS.md).
...
function buildRemovalsList(detection: LegacyDetectionResult): Array<{ path: string; explanation: string }> {
  ...
  // Slash command directories (these are 100% OpenSpecPowers-managed)
  ...
  // Slash command files (these are 100% OpenSpecPowers-managed)
  for (const file of detection.slashCommandFiles) {
    removals.push({ path: file, explanation: 'replaced by skills/' });
  }

  // openspecpowers/AGENTS.md (inside openspecpowers/, it's OpenSpecPowers-managed)
  if (detection.hasOpenspecAgents) {
    removals.push({ path: 'openspecpowers/AGENTS.md', explanation: 'obsolete workflow file' });
  }
...
function buildUpdatesList(detection: LegacyDetectionResult): Array<{ path: string; explanation: string }> {
  ...
  for (const file of detection.configFilesToUpdate) {
    updates.push({ path: file, explanation: 'removing OpenSpecPowers markers' });
  }
...
export function formatDetectionSummary(detection: LegacyDetectionResult): string {
  ...
  lines.push(chalk.bold('Upgrading to the new OpenSpecPowers'));
  lines.push('');
  lines.push('OpenSpecPowers now uses agent skills, the emerging standard across coding');
  ...
  if (updates.length > 0) {
    ...
    lines.push(chalk.dim('OpenSpecPowers markers will be removed, your content preserved:'));
  ...
  for (const file of detection.slashCommandFiles) {
    ...
    for (const [toolId, pattern] of Object.entries(LEGACY_SLASH_COMMAND_PATHS)) {
      if (pattern.type === 'files' && pattern.pattern) {
        ...
        // e.g., '.cursor/commands/openspecpowers-*.md' -> /^\.cursor\/commands\/openspecpowers-.*\.md$/
        const patterns = Array.isArray(pattern.pattern) ? pattern.pattern : [pattern.pattern];
        let matched = false;
        for (const p of patterns) {
...
export function formatProjectMdMigrationHint(): string {
  ...
  lines.push('  • openspecpowers/project.md');
  lines.push(chalk.dim('    We won\'t delete this file. It may contain useful project context.'));
  lines.push('');
  lines.push(chalk.dim('    The new openspecpowers/config.yaml has a "context:" section for planning'));
  lines.push(chalk.dim('    context. This is included in every OpenSpecPowers request and works more'));
