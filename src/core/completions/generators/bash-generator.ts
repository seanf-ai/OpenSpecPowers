...
/**
 * Generates Bash completion scripts for the OpenSpecPowers CLI.
...
export class BashGenerator implements CompletionGenerator {
  ...
  generate(commands: CommandDefinition[]): string {
    ...
    return `# Bash completion script for OpenSpecPowers CLI
...
${helpers}
complete -F _openspecpowers_completion openspecpowers
`;
  }
  ...
  private generatePositionalCompletion(positionalType: string | undefined, indent: string): string[] {
    ...
    switch (positionalType) {
      case 'change-id':
        lines.push(`${indent}_openspecpowers_complete_changes`);
        break;
      case 'spec-id':
        lines.push(`${indent}_openspecpowers_complete_specs`);
        break;
      case 'change-or-spec-id':
        lines.push(`${indent}_openspecpowers_complete_items`);
        break;
      case 'shell':
