...
# Use openspecpowers __complete to get available changes
_openspecpowers_complete_changes() {
  local -a changes
  while IFS=$'\\t' read -r id desc; do
    changes+=("$id:$desc")
  done < <(openspecpowers __complete changes 2>/dev/null)
  _describe "change" changes
}

# Use openspecpowers __complete to get available specs
_openspecpowers_complete_specs() {
  local -a specs
  while IFS=$'\\t' read -r id desc; do
    specs+=("$id:$desc")
  done < <(openspecpowers __complete specs 2>/dev/null)
