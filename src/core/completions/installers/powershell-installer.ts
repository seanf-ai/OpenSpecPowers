...
export class PowerShellInstaller {
  ...
  getInstallationPath(): string {
    ...
    return path.join(profileDir, 'OpenSpecPowersCompletion.ps1');
  }
  ...
  async configureProfile(scriptPath: string): Promise<boolean> {
    ...
    for (const profilePath of profilePaths) {
      try {
        ...
        // Add OpenSpecPowers completion configuration with markers
        const openspecpowersBlock = [
          '',
        ...
        const newContent = profileContent + openspecpowersBlock;
  ...
  async removeProfileConfig(): Promise<boolean> {
    ...
    for (const profilePath of profilePaths) {
      try {
        ...
        if (startIndex === -1) {
          continue; // No OpenSpecPowers block found
        }
  ...
  private generateInstructions(installedPath: string): string[] {
    ...
    return [
      ...
      '',
      '  # Source OpenSpecPowers completions',
