...
export class ViewCommand {
  async execute(targetPath: string = '.'): Promise<void> {
    const openspecpowersDir = path.join(targetPath, 'openspecpowers');
    
    if (!fs.existsSync(openspecpowersDir)) {
      console.error(chalk.red('No openspecpowers directory found'));
      process.exit(1);
    }

    console.log(chalk.bold('\nOpenSpecPowers Dashboard\n'));
    console.log('═'.repeat(60));

    // Get changes and specs data
    const changesData = await this.getChangesData(openspecpowersDir);
    const specsData = await this.getSpecsData(openspecpowersDir);
    ...
    console.log('\n' + '═'.repeat(60));
    console.log(chalk.dim(`\nUse ${chalk.white('openspecpowers list --changes')} or ${chalk.white('openspecpowers list --specs')} for detailed views`));
  }

  private async getChangesData(openspecpowersDir: string): Promise<{
    draft: Array<{ name: string }>;
    active: Array<{ name: string; progress: { total: number; completed: number } }>;
    completed: Array<{ name: string }>;
  }> {
    const changesDir = path.join(openspecpowersDir, 'changes');
  ...
  private async getSpecsData(openspecpowersDir: string): Promise<Array<{ name: string; requirementCount: number }>> {
    const specsDir = path.join(openspecpowersDir, 'specs');
