 ...
 * Init Command
 *
 * Sets up OpenSpecPowers with Agent Skills and /opsp:* slash commands.
...
const WORKFLOW_TO_SKILL_DIR: Record<string, string> = {
  'explore': 'openspecpowers-explore',
  'new': 'openspecpowers-new-change',
  'continue': 'openspecpowers-continue-change',
  'apply': 'openspecpowers-apply-change',
  'ff': 'openspecpowers-ff-change',
  'sync': 'openspecpowers-sync-specs',
  'archive': 'openspecpowers-archive-change',
  'bulk-archive': 'openspecpowers-bulk-archive-change',
  'verify': 'openspecpowers-verify-change',
  'onboard': 'openspecpowers-onboard',
  'propose': 'openspecpowers-propose',
};
...
export class InitCommand {
  ...
  async execute(targetPath: string): Promise<void> {
    ...
    const openspecpowersDir = OPENSPEC_DIR_NAME;
    const openspecpowersPath = path.join(projectPath, openspecpowersDir);

    // Validation happens silently in the background
    const extendMode = await this.validate(projectPath, openspecpowersPath);
    ...
    await this.createDirectoryStructure(openspecpowersPath, extendMode);

    // Generate skills and commands for each tool
    const results = await this.generateSkillsAndCommands(projectPath, validatedTools);

    // Create config.yaml if needed
    const configStatus = await this.createConfig(openspecpowersPath, extendMode);
  ...
  private async validate(
    projectPath: string,
    openspecpowersPath: string
  ): Promise<boolean> {
    const extendMode = await FileSystemUtils.directoryExists(openspecpowersPath);
  ...
  private async handleLegacyCleanup(projectPath: string, extendMode: boolean): Promise<void> {
    ...
    if (this.force || !canPrompt) {
      ...
      // Legacy slash commands are 100% OpenSpecPowers-managed, and config file cleanup
  ...
  private async getSelectedTools(
    ...
    if (configuredNames.length > 0) {
      console.log(`OpenSpecPowers configured: ${configuredNames.join(', ')} (pre-selected)`);
    }
  ...
  private async createDirectoryStructure(openspecpowersPath: string, extendMode: boolean): Promise<void> {
    if (extendMode) {
      // In extend mode, just ensure directories exist without spinner
      const directories = [
        openspecpowersPath,
        path.join(openspecpowersPath, 'specs'),
        path.join(openspecpowersPath, 'changes'),
        path.join(openspecpowersPath, 'changes', 'archive'),
      ];
    ...
    const spinner = this.startSpinner('Creating OpenSpecPowers structure...');
    ...
    spinner.stopAndPersist({
      ...
      text: PALETTE.white('OpenSpecPowers structure created'),
    });
  }
  ...
  private async createConfig(openspecpowersPath: string, extendMode: boolean): Promise<'created' | 'exists' | 'skipped'> {
    const configPath = path.join(openspecpowersPath, 'config.yaml');
    const configYmlPath = path.join(openspecpowersPath, 'config.yml');
  ...
  private displaySuccessMessage(
    ...
    console.log();
    console.log(chalk.bold('OpenSpecPowers Setup Complete'));
    console.log();
    ...
    if (configStatus === 'created') {
      console.log(`Config: openspecpowers/config.yaml (schema: ${DEFAULT_SCHEMA})`);
    } else if (configStatus === 'exists') {
      // Show actual filename (config.yaml or config.yml)
      const configYaml = path.join(projectPath, OPENSPEC_DIR_NAME, 'config.yaml');
      const configYml = path.join(projectPath, OPENSPEC_DIR_NAME, 'config.yml');
      const configName = fs.existsSync(configYaml) ? 'config.yaml' : fs.existsSync(configYml) ? 'config.yml' : 'config.yaml';
      console.log(`Config: openspecpowers/${configName} (exists)`);
    } else {
    ...
    if (activeWorkflows.includes('propose')) {
      ...
      console.log('  Start your first change: /opsp:propose "your idea"');
    } else if (activeWorkflows.includes('new')) {
      console.log(chalk.bold('Getting started:'));
      console.log('  Start your first change: /opsp:new "your idea"');
    } else {
      console.log("Done. Run 'openspecpowers config profile' to configure your workflows.");
    }

    // Links
    console.log();
    console.log(`Learn more: ${chalk.cyan('https://github.com/seanf-ai/OpenSpecPowers')}`);
    console.log(`Feedback:   ${chalk.cyan('https://github.com/seanf-ai/OpenSpecPowers/issues')}`);
