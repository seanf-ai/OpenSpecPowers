...
export class ArchiveCommand {
  async execute(
    ...
    const targetPath = '.';
    const changesDir = path.join(targetPath, 'openspecpowers', 'changes');
    const archiveDir = path.join(changesDir, 'archive');
    const mainSpecsDir = path.join(targetPath, 'openspecpowers', 'specs');

    // Check if changes directory exists
    try {
      await fs.access(changesDir);
    } catch {
      throw new Error("No OpenSpecPowers changes directory found. Run 'openspecpowers init' first.");
    }
