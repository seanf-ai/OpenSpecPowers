 ...
 * Update Command
 *
 * Refreshes OpenSpecPowers skills and commands for configured tools.
...
export class UpdateCommand {
  ...
  async execute(projectPath: string): Promise<void> {
    ...
    const openspecpowersPath = path.join(resolvedProjectPath, OPENSPEC_DIR_NAME);
    ...
    // Use detected tool directories to preserve existing opsp skills/commands.
    ...
    if (configuredTools.length === 0 && newlyConfiguredTools.length === 0) {
      ...
      console.log(chalk.dim('Run "openspecpowers init" to set up tools.'));
      return;
    }
    ...
    if (newlyConfiguredTools.length > 0) {
      ...
      console.log('  /opsp:new       Start a new change');
      console.log('  /opsp:continue  Create the next artifact');
      console.log('  /opsp:apply     Implement tasks');
      console.log();
  ...
  private detectNewTools(projectPath: string, configuredTools: string[]): void {
    ...
    if (newTools.length > 0) {
      ...
      console.log(
        chalk.yellow(
          `Detected new ${toolNoun}: ${newToolNames.join(', ')}. Run 'openspecpowers init' to add ${pronoun}.`
        )
      );
    }
  }
  ...
  private displayExtraWorkflowsNote(
    ...
    if (extraWorkflows.length > 0) {
      console.log(chalk.dim(`Note: ${extraWorkflows.length} extra workflows not in profile (use \`openspecpowers config profile\` to manage)`));
    }
  }
  ...
  /**
   * Detect and handle legacy OpenSpecPowers artifacts.
