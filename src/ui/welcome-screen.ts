...
function getWelcomeText(): string[] {
  return [
    chalk.white.bold('Welcome to OpenSpecPowers'),
    chalk.dim('A lightweight spec-driven framework'),
    '',
    chalk.white('This setup will configure:'),
    chalk.dim('  • Agent Skills for AI tools'),
    chalk.dim('  • /opsp:* slash commands'),
    '',
    chalk.white('Quick start after setup:'),
    `  ${chalk.yellow('/opsp:new')}      ${chalk.dim('Create a change')}`,
    `  ${chalk.yellow('/opsp:continue')} ${chalk.dim('Next artifact')}`,
    `  ${chalk.yellow('/opsp:apply')}    ${chalk.dim('Implement tasks')}`,
    '',
