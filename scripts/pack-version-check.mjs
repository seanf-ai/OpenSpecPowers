...
function main() {
  ...
  try {
    log(`Packing @seanf-ai/openspecpowers@${expected}...`);
    const filename = npmPack();
    tgzPath = path.resolve(filename);
    log(`Created: ${tgzPath}`);

    work = mkdtempSync(path.join(tmpdir(), 'openspecpowers-pack-check-'));
    log(`Temp dir: ${work}`);
    ...
    const binRel = path.join('node_modules', '@fission-ai', 'openspecpowers', 'bin', 'openspecpowers.js');
