...
- [#747](https://github.com/seanf-ai/OpenSpecPowers/pull/747) [`1e94443`](https://github.com/seanf-ai/OpenSpecPowers/commit/1e94443a3551b228eecbc89e95d96d3b9600a192) Thanks [@TabishB](https://github.com/TabishB)! - ### New Features

  - **Profile system** — Choose between `core` (4 essential workflows) and `custom` (pick any subset) profiles to control which skills get installed. Manage profiles with the new `openspecpowers config profile` command
  ...
  - **AI tool auto-detection** — `openspecpowers init` now scans your project for existing tool directories (`.claude/`, `.cursor/`, etc.) and pre-selects detected tools
  ...
  - **Sync prunes deselected workflows** — `openspecpowers update` now removes command files and skill directories for workflows you've deselected, keeping your project clean
  - **Config drift warning** — `openspecpowers config list` warns when global config is out of sync with the current project
...
- [#627](https://github.com/seanf-ai/OpenSpecPowers/pull/627) [`afb73cf`](https://github.com/seanf-ai/OpenSpecPowers/commit/afb73cf9ec59c6f8b26d0c538c0218c203ba3c56) Thanks [@TabishB](https://github.com/TabishB)! - ### Bug Fixes

  - **OpenCode command references** — Command references in generated files now use the correct `/opsp-` hyphen format instead of `/opsp:` colon format, ensuring commands work properly in OpenCode

## 1.1.0

### Minor Changes

- [#625](https://github.com/seanf-ai/OpenSpecPowers/pull/625) [`53081fb`](https://github.com/seanf-ai/OpenSpecPowers/commit/53081fb2a26ec66d2950ae0474b9a56cbc5b5a76) Thanks [@TabishB](https://github.com/TabishB)! - ### Bug Fixes
...
- [#550](https://github.com/seanf-ai/OpenSpecPowers/pull/550) [`86d2e04`](https://github.com/seanf-ai/OpenSpecPowers/commit/86d2e04cae76a999dbd1b4571f52fa720036be0c) Thanks [@jerome-benoit](https://github.com/jerome-benoit)! - ### Improvements
...
- [#596](https://github.com/seanf-ai/OpenSpecPowers/pull/596) [`e91568d`](https://github.com/seanf-ai/OpenSpecPowers/commit/e91568deb948073f3e9d9bb2d2ab5bf8080d6cf4) Thanks [@TabishB](https://github.com/TabishB)! - ### Bug Fixes
...
- [#587](https://github.com/seanf-ai/OpenSpecPowers/pull/587) [`943e0d4`](https://github.com/seanf-ai/OpenSpecPowers/commit/943e0d41026d034de66b9442d1276c01b293eb2b) Thanks [@TabishB](https://github.com/TabishB)! - ### Bug Fixes

  - Fixed incorrect archive path in onboarding documentation — the template now shows the correct path `openspecpowers/changes/archive/YYYY-MM-DD-<name>/` instead of the incorrect `openspecpowers/archive/YYYY-MM-DD--<name>/`

## 1.0.0

### Major Changes

- [#578](https://github.com/seanf-ai/OpenSpecPowers/pull/578) [`0cc9d90`](https://github.com/seanf-ai/OpenSpecPowers/commit/0cc9d9025af367faa1688a7b2606a2549053cd3f) Thanks [@TabishB](https://github.com/TabishB)! - ## OpenSpecPowers 1.0 — The OPSP Release

  The workflow has been rebuilt from the ground up. OPSP replaces the old phase-locked `/openspecpowers:*` commands with an action-based system where AI understands what artifacts exist, what's ready to create, and what each action unlocks.

  ### Breaking Changes

  - **Old commands removed** — `/openspecpowers:proposal`, `/openspecpowers:apply`, and `/openspecpowers:archive` no longer exist
  - **Config files removed** — Tool-specific instruction files (`CLAUDE.md`, `.cursorrules`, `AGENTS.md`, `project.md`) are no longer generated
  - **Migration** — Run `openspecpowers init` to upgrade. Legacy artifacts are detected and cleaned up with confirmation.
  ...
  **Now:** Flexible actions on a change. Edit any artifact anytime. The artifact graph tracks state automatically.
  ...
  | `/opsp:continue`     | Create one artifact at a time (step-through)         |
  | `/opsp:ff`           | Create all planning artifacts at once (fast-forward) |
  | `/opsp:apply`        | Implement tasks                                      |
  | `/opsp:verify`       | Validate implementation matches artifacts            |
  | `/opsp:sync`         | Sync delta specs to main specs                       |
  | `/opsp:archive`      | Archive completed change                             |
  | `/opsp:bulk-archive` | Archive multiple changes with conflict detection     |
  | `/opsp:onboard`      | Guided 15-minute walkthrough of complete workflow    |
  ...
  **Now:** Single `.claude/skills/` directory with YAML-fronted markdown files. Auto-detected by Claude Code, Cursor, Windsurf. Cross-editor compatible.
  ...
  - **Onboarding skill** — `/opsp:onboard` walks new users through their first complete change with codebase-aware task suggestions and step-by-step narration (11 phases, ~15 minutes)
  ...
  - **Interactive setup** — `openspecpowers init` shows animated welcome screen and searchable multi-select for choosing tools. Pre-selects already-configured tools for easy refresh.

  - **Customizable schemas** — Define custom artifact workflows in `openspecpowers/schemas/` without touching package code. Teams can share workflows via version control.
  ...
  - Added migration guide for upgrading from pre-OPSP versions

## 0.23.0

### Minor Changes

- [#540](https://github.com/seanf-ai/OpenSpecPowers/pull/540) [`c4cfdc7`](https://github.com/seanf-ai/OpenSpecPowers/commit/c4cfdc7c499daef30d8a218f5f59b8d9e5adb754) Thanks [@TabishB](https://github.com/TabishB)! - ### New Features

  - **Bulk archive skill** — Archive multiple completed changes in a single operation with `/opsp:bulk-archive`. Includes batch validation, spec conflict detection, and consolidated confirmation
...
- [#530](https://github.com/seanf-ai/OpenSpecPowers/pull/530) [`33466b1`](https://github.com/seanf-ai/OpenSpecPowers/commit/33466b1e2a6798bdd6d0e19149173585b0612e6f) Thanks [@TabishB](https://github.com/TabishB)! - Add project-level configuration, project-local schemas, and schema management commands

  **New Features**

  - **Project-level configuration** — Configure OpenSpecPowers behavior per-project via `openspecpowers/config.yaml`, including custom rules injection, context files, and schema resolution settings
  - **Project-local schemas** — Define custom artifact schemas within your project's `openspecpowers/schemas/` directory for project-specific workflows
  - **Schema management commands** — New `openspecpowers schema` commands (`list`, `show`, `export`, `validate`) for inspecting and managing artifact schemas (experimental)
...
- [#516](https://github.com/seanf-ai/OpenSpecPowers/pull/516) [`b5a8847`](https://github.com/seanf-ai/OpenSpecPowers/commit/b5a884748be6156a7bb140b4941cfec4f20a9fc8) Thanks [@TabishB](https://github.com/TabishB)! - Add feedback command and Nix flake support

  **New Features**

  - **Feedback command** — Submit feedback directly from the CLI with `openspecpowers feedback`, which creates GitHub Issues with automatic metadata inclusion and graceful fallback for manual submission
  - **Nix flake support** — Install and develop openspecpowers using Nix with the new `flake.nix`, including automated flake maintenance and CI validation
  ...
  **Other**
  ...
  - Improved change inference in `opsp apply` — automatically detects the target change from conversation context or prompts when ambiguous
...
- [#502](https://github.com/seanf-ai/OpenSpecPowers/pull/502) [`9db74aa`](https://github.com/seanf-ai/OpenSpecPowers/commit/9db74aa5ac6547efadaed795217cfa17444f2004) Thanks [@TabishB](https://github.com/TabishB)! - Add `/opsp:verify` command and fix vitest process storms

  **New Features**

  - **`/opsp:verify` command** — Validate that change implementations match their specifications
...
- eb152eb: Add Continue IDE support, shell completions, and `/opsp:explore` command

  **New Features**

  - **Continue IDE support** – OpenSpecPowers now generates slash commands for [Continue](https://continue.dev/), expanding editor integration options alongside Cursor, Windsurf, Claude Code, and others
  - **Shell completions for Bash, Fish, and PowerShell** – Run `openspecpowers completion install` to set up tab completion in your preferred shell
  - **`/opsp:explore` command** – A new thinking partner mode for exploring ideas and investigating problems before committing to changes
  ...
  **Other**
  ...
  - Added optional anonymous usage statistics to help understand how OpenSpecPowers is used. This is **opt-out** by default – set `OPENSPEC_TELEMETRY=0` or `DO_NOT_TRACK=1` to disable. Only command names and version are collected; no arguments, file paths, or content. Automatically disabled in CI environments.

## 0.18.0

### Minor Changes

- 8dfd824: Add OPSP experimental workflow commands and enhanced artifact system

  **New Commands:**

  - `/opsp:ff` - Fast-forward through artifact creation, generating all needed artifacts in one go
  - `/opsp:sync` - Sync delta specs from a change to main specs
  - `/opsp:archive` - Archive completed changes with smart sync check
  ...
  **Artifact Workflow Enhancements:**
  ...
  - Per-change schema metadata via `.openspecpowers.yaml` files
...
- 2e71835: Add `openspecpowers config` command and Oh-my-zsh completions

  **New Features**

  - Add `openspecpowers config` command for managing global configuration settings
...
- Add Continue slash command support so `openspecpowers init` can generate `.continue/prompts/openspecpowers-*.prompt` files with MARKDOWN frontmatter and `$ARGUMENTS` placeholder, and refresh them on `openspecpowers update`.

- Add Antigravity slash command support so `openspecpowers init` can generate `.agent/workflows/openspecpowers-*.md` files with description-only frontmatter and `openspecpowers update` refreshes existing workflows alongside Windsurf.
...
- 4758c5c: Add support for new AI tools with native slash command integration
  ...
  - **Gemini CLI**: Add native TOML-based slash command support for Gemini CLI with `.gemini/commands/openspecpowers/` integration
...
- 8386b91: Add support for new AI assistants and configuration improvements
  ...
  - fix: recreate missing openspecpowers template files in extend mode
...
- 082abb4: Add factory function support for slash commands and non-interactive init options
  ...
  - **Non-interactive init options**: Added `--tools`, `--all-tools`, and `--skip-tools` CLI flags to `openspecpowers init` for automated initialization in CI/CD pipelines while maintaining backward compatibility with interactive mode

## 0.11.0

### Minor Changes

- 312e1d6: Add Amazon Q Developer CLI integration. OpenSpecPowers now supports Amazon Q Developer with automatic prompt generation in `.amazonq/prompts/` directory, allowing you to use OpenSpecPowers slash commands with Amazon Q's @-syntax.
...
- 2ae0484: Fix cross-platform path handling issues. This release includes fixes for joinPath behavior and slash command path resolution to ensure OpenSpecPowers works correctly across all platforms.

## 0.9.1

### Patch Changes

- 8210970: Fix OpenSpecPowers not working on Windows when Codex integration is selected. This release includes fixes for cross-platform path handling and normalization to ensure OpenSpecPowers works correctly on Windows systems.
...
- Add GitHub Copilot slash command support. OpenSpecPowers now writes prompts to `.github/prompts/openspecpowers-{proposal,apply,archive}.prompt.md` with YAML frontmatter and `$ARGUMENTS` placeholder, and refreshes them on `openspecpowers update`.

## 0.8.1

### Patch Changes

- d070d08: Fix CLI version mismatch and add a release guard that validates the packed tarball prints the same version as package.json via `openspecpowers --version`.
...
- Add Codex slash command support. OpenSpecPowers now writes prompts directly to Codex's global directory (`~/.codex/prompts` or `$CODEX_HOME/prompts`) and refreshes them on `openspecpowers update`.

## 0.7.0

### Minor Changes

- Add native Kilo Code workflow integration so `openspecpowers init` and `openspecpowers update` manage `.kilocode/workflows/openspecpowers-*.md` files.
...
- Add OpenSpecPowers change proposals for CLI improvements and enhanced user experience
...
- Enhance `openspecpowers init` with extend mode, multi-tool selection, and an interactive `AGENTS.md` configurator.

## 0.2.0

### Minor Changes

- ce5cead: - Add an `openspecpowers view` dashboard that rolls up spec counts and change progress at a glance
  - Generate and update AI slash commands alongside the renamed `openspecpowers/AGENTS.md` instructions file
  - Remove the deprecated `openspecpowers diff` command and direct users to `openspecpowers show`
