...
## Purpose
Define how `openspecpowers/config.yaml` is discovered, parsed, validated, and exposed to callers with safe fallbacks.

## Requirements
### Requirement: Load project config from openspecpowers/config.yaml

The system SHALL read and parse the project configuration file located at `openspecpowers/config.yaml` relative to the project root.

#### Scenario: Valid config file exists
- **WHEN** `openspecpowers/config.yaml` exists with valid YAML content
- **THEN** system parses the file and returns a ProjectConfig object

#### Scenario: Config file does not exist
- **WHEN** `openspecpowers/config.yaml` does not exist
- **THEN** system returns null without error

#### Scenario: Config file has invalid YAML syntax
- **WHEN** `openspecpowers/config.yaml` contains malformed YAML
- **THEN** system logs a warning message and returns null

#### Scenario: Config file has valid YAML but invalid schema
- **WHEN** `openspecpowers/config.yaml` contains valid YAML that fails Zod schema validation
...
- **WHEN** `openspecpowers/config.yml` exists and `openspecpowers/config.yaml` does not exist
- **THEN** system reads from `openspecpowers/config.yml`

#### Scenario: Both .yaml and .yml exist
- **WHEN** both `openspecpowers/config.yaml` and `openspecpowers/config.yml` exist
- **THEN** system prefers `openspecpowers/config.yaml`
...
- **WHEN** config file has syntax errors and user runs `openspecpowers new change`
