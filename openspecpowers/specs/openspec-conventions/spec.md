# OpenSpecPowers Conventions Specification

## Purpose

OpenSpecPowers conventions SHALL define how system capabilities are documented, how changes are proposed and tracked, and how specifications evolve over time. This meta-specification serves as the source of truth for OpenSpecPowers's own conventions.
## Requirements
### Requirement: Structured conventions for specs and changes

OpenSpecPowers conventions SHALL mandate a structured spec format with clear requirement and scenario sections so tooling can parse consistently.

#### Scenario: Following the structured spec format

- **WHEN** writing or updating OpenSpecPowers specifications
...
OpenSpecPowers specifications SHALL capture verifiable behavior contracts and avoid internal implementation detail.
...
OpenSpecPowers conventions SHALL keep specs lightweight by default and scale rigor only when risk or coordination complexity demands it.
...
An OpenSpecPowers project SHALL maintain a consistent directory structure for specifications and changes.

#### Scenario: Initializing project structure
- **WHEN** an OpenSpecPowers project is initialized
...
OpenSpecPowers CLI design SHALL use verbs as top-level commands with nouns provided as arguments or flags for scoping.

#### Scenario: Verb-first command discovery
- **WHEN** a user runs a command like `openspecpowers list`
...
- **WHEN** users run noun-prefixed commands such as `openspecpowers spec ...` or `openspecpowers change ...`
...
- **THEN** `openspecpowers show` and `openspecpowers validate` SHALL accept `--type spec|change`
