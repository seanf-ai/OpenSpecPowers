# opsp-verify-skill Specification

## Purpose
Define `/opsp:verify` behavior for assessing implementation completeness, correctness, and coherence against change artifacts.

## Requirements
### Requirement: Verify Skill Invocation
The system SHALL provide an `/opsp:verify` skill that validates implementation against change artifacts.

#### Scenario: Verify with change name provided
- **WHEN** agent executes `/opsp:verify <change-name>`
...
- **WHEN** agent executes `/opsp:verify` without a change name
...
- **AND** suggests running `/opsp:continue` to create tasks
...
- **WHEN** verifying completeness
- **AND** delta specs exist in `openspecpowers/changes/<name>/specs/`
