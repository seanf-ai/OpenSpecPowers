# OPSP Archive Skill Spec

## Purpose

Define the expected behavior for the `/opsp:archive` skill, including readiness checks, spec sync prompting, archive execution, and user-facing output.

## Requirements

### Requirement: OPSP Archive Skill

The system SHALL provide an `/opsp:archive` skill that archives completed changes in the experimental workflow.

#### Scenario: Archive a change with all artifacts complete

- **WHEN** agent executes `/opsp:archive` with a change name
- **AND** all artifacts in the schema are complete
- **AND** all tasks are complete
- **THEN** the agent moves the change to `openspecpowers/changes/archive/YYYY-MM-DD-<name>/`
- **AND** displays success message with archived location

#### Scenario: Change selection prompt

- **WHEN** agent executes `/opsp:archive` without specifying a change
...
- **AND** if user confirms, execute `/opsp:sync` logic
...
- **AND** preserve `.openspecpowers.yaml` file in archived change
...
- **THEN** display summary:
  - Specs synced (from `/opsp:sync` output)
