# opsp-onboard-skill Specification

## Purpose
Define `/opsp:onboard` behavior for guiding users through an end-to-end OpenSpecPowers workflow on their real codebase.

## Requirements
### Requirement: OPSP Onboard Skill

The system SHALL provide an `/opsp:onboard` skill that guides users through their first complete OpenSpecPowers workflow cycle with narration and real codebase work.

#### Scenario: Skill invocation

- **WHEN** user invokes `/opsp:onboard`
- **THEN** agent checks if OpenSpecPowers is initialized
- **AND** if not initialized, prompts user to run `openspecpowers init` first
...
- **THEN** agent briefly demonstrates `/opsp:explore` by investigating relevant code
...
- **THEN** agent runs `openspecpowers new change "<name>"` with derived kebab-case name
...
- **AND** provides command reference table (/opsp:explore, /opsp:new, /opsp:ff, /opsp:continue, /opsp:apply, /opsp:verify, /opsp:archive)
- **AND** suggests next actions (try /opsp:new or /opsp:ff on something)
...
- **AND** explains how to continue later with `/opsp:continue <name>`
...
- **AND** exits gracefully with encouragement to try `/opsp:new`
