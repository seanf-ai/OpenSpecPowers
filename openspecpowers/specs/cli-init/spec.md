...
The command SHALL create the OpenSpecPowers directory structure with config file.
...
- **THEN** display animated welcome screen with OpenSpecPowers logo
...
- **AND** generate slash commands in `.<tool>/commands/opsp/` directory for each selected tool
- **AND** create `openspecpowers/config.yaml` with default schema setting
...
- **WHEN** the `openspecpowers/` directory already exists
- **THEN** inform the user that OpenSpecPowers is already initialized, skip recreating the base structure, and enter an extend mode
...
- **AND** display getting started section with:
  ...
  - `/opsp:continue` - Create the next artifact
...
`openspecpowers init` SHALL allow users to add configuration files for new AI coding assistants after the initial setup.

#### Scenario: Configuring an extra tool after initial setup
- **GIVEN** an `openspecpowers/` directory already exists and at least one AI tool file is present
- **WHEN** the user runs `openspecpowers init` and selects a different supported AI tool
- **THEN** generate that tool's configuration files with OpenSpecPowers markers the same way as during first-time initialization
...
`openspecpowers init` SHALL summarize tool actions when initialization or extend mode completes.
...
`openspecpowers init` SHALL treat extend mode without new native tool selections as a successful refresh.

#### Scenario: Allowing empty extend runs
- **WHEN** OpenSpecPowers is already initialized and the user selects no additional natively supported tools
- **THEN** complete successfully without requiring additional tool setup
- **AND** preserve the existing OpenSpecPowers structure and config files
...
- **THEN** create only the openspecpowers directory structure
...
- **WHEN** user runs `openspecpowers init` without `--tools`
- **THEN** fail with exit code 1
...
The command SHALL generate opsp slash commands for selected AI tools.
...
The command SHALL create an OpenSpecPowers config file with schema settings.

#### Scenario: Creating config.yaml

- **WHEN** initialization completes
- **AND** config.yaml does not exist
- **THEN** create `openspecpowers/config.yaml` with default schema setting
- **AND** display config location in output

#### Scenario: Preserving existing config.yaml

- **WHEN** initialization runs in extend mode
- **AND** `openspecpowers/config.yaml` already exists
...
#### Scenario: Running openspecpowers experimental

- **WHEN** user runs `openspecpowers experimental`
- **THEN** delegate to `openspecpowers init`
