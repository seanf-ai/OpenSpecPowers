...
The `openspecpowers list` command SHALL provide developers with a quick overview of all active changes in the project, showing their names and task completion status.
## Requirements
### Requirement: Command Execution
The command SHALL scan and analyze either active changes or specs based on the selected mode.

#### Scenario: Scanning for changes (default)
- **WHEN** `openspecpowers list` is executed without flags
- **THEN** scan the `openspecpowers/changes/` directory for change directories
- **AND** exclude the `archive/` subdirectory from results
- **AND** parse each change's `tasks.md` file to count task completion

#### Scenario: Scanning for specs
- **WHEN** `openspecpowers list --specs` is executed
- **THEN** scan the `openspecpowers/specs/` directory for capabilities
...
- **WHEN** `openspecpowers/changes/` directory doesn't exist
- **THEN** display error: "No OpenSpecPowers changes directory found. Run 'openspecpowers init' first."
...
This command provides that visibility with minimal effort, following OpenSpecPowers's philosophy of simplicity and clarity.