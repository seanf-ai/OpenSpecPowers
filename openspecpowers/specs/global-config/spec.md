...
- **THEN** `getGlobalConfigDir()` returns `/custom/config/openspecpowers`
...
- **THEN** `getGlobalConfigDir()` returns `~/.config/openspecpowers` (expanded to absolute path)

#### Scenario: Windows platform
- **WHEN** the platform is Windows
- **AND** `%APPDATA%` is set to `C:\Users\User\AppData\Roaming`
- **THEN** `getGlobalConfigDir()` returns `C:\Users\User\AppData\Roaming\openspecpowers`
