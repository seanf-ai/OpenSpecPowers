...
## Purpose
Define top-level `openspecpowers show` behavior for interactive and direct display of change and spec content.
...
- **WHEN** executing `openspecpowers show` without arguments
...
- **WHEN** executing `openspecpowers show` without arguments
- **THEN** do not prompt
- **AND** print a helpful hint with examples for `openspecpowers show <item>` or `openspecpowers change/spec show`
- **AND** exit with code 1

#### Scenario: Direct item display

- **WHEN** executing `openspecpowers show <item-name>`
...
- **AND** if it matches both, print an ambiguity error and suggest `--type change|spec` or using `openspecpowers change show`/`openspecpowers spec show`
- **AND** if it matches neither, print not-found with nearest-match suggestions

#### Scenario: Explicit type override

- **WHEN** executing `openspecpowers show --type change <item>`
- **THEN** treat `<item>` as a change ID and show it (skipping auto-detection)

- **WHEN** executing `openspecpowers show --type spec <item>`
...
- **WHEN** executing `openspecpowers show <item> --json`
...
- **WHEN** showing a change with `openspecpowers show <change-name> --deltas-only`
...
- **WHEN** showing a spec with `openspecpowers show <spec-id> --requirements`
