...
## Purpose
Define `openspecpowers validate` behavior for validating changes and specs with actionable remediation guidance and structured output.
...
- **THEN** show error "No deltas found" with guidance:
  ...
  - Remind authors that files must live under `openspecpowers/changes/{id}/specs/<capability>/spec.md`
  - Include an explicit note: "Spec delta files cannot start with titles before the operation headers"
  - Suggest running `openspecpowers change show {id} --json --deltas-only` for debugging
...
- **THEN** include expected header names and a minimal skeleton:
  ...
  - Mention the quick-reference section in `openspecpowers/AGENTS.md` as the authoritative template
...
- **THEN** emit an error explaining that `### Requirement:` lines must be followed by narrative text before any `#### Scenario:` headers
  ...
  - Reference the pre-validation checklist in `openspecpowers/AGENTS.md`
...
- Source file path (`openspecpowers/changes/{id}/proposal.md`, `.../specs/{cap}/spec.md`)
...
- **THEN** print "Next steps" with 2-3 targeted bullets and suggest `openspecpowers change show <id> --json --deltas-only`
...
- **WHEN** executing `openspecpowers validate` without arguments
...
- **WHEN** executing `openspecpowers validate <item-name>`
...
- **WHEN** executing `openspecpowers validate --all`
- **THEN** validate all changes in openspecpowers/changes/ (excluding archive)
- **AND** validate all specs in openspecpowers/specs/
...
- **THEN** include all change proposals under `openspecpowers/changes/`
- **AND** exclude the `openspecpowers/changes/archive/` directory

- **WHEN** validating with `--specs`
- **THEN** include all specs that have a `spec.md` under `openspecpowers/specs/<id>/spec.md`

#### Scenario: Validate all changes

- **WHEN** executing `openspecpowers validate --changes`
- **THEN** validate all changes in openspecpowers/changes/ (excluding archive)
...
- **WHEN** executing `openspecpowers validate --specs`
- **THEN** validate all specs in openspecpowers/specs/
...
- **WHEN** executing `openspecpowers validate --all --strict`
...
- **WHEN** executing `openspecpowers validate --all --json`
...
- **WHEN** executing `openspecpowers validate --all --json` (or `--changes` / `--specs`)
...
- **AND** suggest passing `--type change` or `--type spec`, or using `openspecpowers change validate` / `openspecpowers spec validate`
...
- **WHEN** executing `openspecpowers validate --type change <item>`
- **THEN** treat `<item>` as a change ID and validate it (skipping auto-detection)

- **WHEN** executing `openspecpowers validate --type spec <item>`
...
- **WHEN** `openspecpowers validate` is executed with `--no-interactive` or with environment `OPEN_SPEC_INTERACTIVE=0`
...
- **WHEN** running `openspecpowers validate <change-id>`
