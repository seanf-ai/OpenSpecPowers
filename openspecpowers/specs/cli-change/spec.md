...
## Purpose
Define `openspecpowers change` command behavior for showing, listing, and validating change proposals and deltas.
...
- **WHEN** executing `openspecpowers change show update-error --json`
...
- **WHEN** executing `openspecpowers change list`
- **THEN** scan the openspecpowers/changes directory
- **AND** return list of all pending changes
- **AND** support JSON output with `--json` flag

#### Scenario: Show only requirement changes

- **WHEN** executing `openspecpowers change show update-error --requirements-only`
...
- **WHEN** executing `openspecpowers change validate update-error`
...
- **WHEN** executing `openspecpowers list`
- **THEN** display current list of changes (existing behavior)
- **AND** show deprecation notice: "Note: 'openspecpowers list' is deprecated. Use 'openspecpowers change list' instead."

#### Scenario: Legacy list with --all flag

- **WHEN** executing `openspecpowers list --all`
...
- **WHEN** executing `openspecpowers change show` without arguments
...
- **WHEN** executing `openspecpowers change show` without a change name
...
- **WHEN** executing `openspecpowers change validate` without arguments
- **THEN** display an interactive list of available changes
...
- **GIVEN** stdin is not a TTY or `--no-interactive` is provided or environment variable `OPEN_SPEC_INTERACTIVE=0`
- **WHEN** executing `openspecpowers change validate` without a change name
- **THEN** do not prompt interactively
- **AND** print the existing hint including available change IDs
- **AND** set `process.exitCode = 1`
