...
- **WHEN** user runs `openspecpowers status --change <id>`
...
- **WHEN** user runs `openspecpowers status --change <id> --json`
...
- **WHEN** user runs `openspecpowers status --change <id>` on a change with no artifacts
...
- **WHEN** user runs `openspecpowers status` without `--change`
...
- **WHEN** user runs `openspecpowers status --change unknown-id`
- **AND** directory `openspecpowers/changes/unknown-id/` does not exist
- **THEN** the system displays an error listing all available change directories

### Requirement: Next Artifact Discovery

The workflow SHALL use `openspecpowers status` output to determine what can be created next, rather than a separate next-command surface.

#### Scenario: Discover next artifacts from status output

- **WHEN** a user needs to know which artifact to create next
- **THEN** `openspecpowers status --change <id>` identifies ready artifacts with `[ ]`
...
- **WHEN** user runs `openspecpowers instructions <artifact> --change <id>`
...
- **WHEN** user runs `openspecpowers instructions <artifact> --change <id> --json`
- **THEN** the system outputs JSON matching ArtifactInstructions interface

#### Scenario: Unknown artifact

- **WHEN** user runs `openspecpowers instructions unknown-artifact --change <id>`
...
- **WHEN** user runs `openspecpowers instructions proposal --change <id>` on a scaffolded change
...
- **WHEN** user runs `openspecpowers templates`
...
- **WHEN** user runs `openspecpowers templates --schema tdd`
- **THEN** the system displays template paths for the specified schema

#### Scenario: Templates JSON output
- **WHEN** user runs `openspecpowers templates --json`
...
- **WHEN** user runs `openspecpowers new change add-feature`
- **THEN** the system creates `openspecpowers/changes/add-feature/` directory

#### Scenario: Invalid change name
- **WHEN** user runs `openspecpowers new change "Add Feature"` with invalid name
- **THEN** the system displays validation error with guidance

#### Scenario: Duplicate change name
- **WHEN** user runs `openspecpowers new change existing-change` for an existing change
- **THEN** the system displays an error indicating the change already exists

#### Scenario: Create with description
- **WHEN** user runs `openspecpowers new change add-feature --description "Add new feature"`
...
- **WHEN** user runs `openspecpowers status --change <id> --schema tdd`
...
The system SHALL generate schema-aware apply instructions via `openspecpowers instructions apply`.

#### Scenario: Generate apply instructions

- **WHEN** user runs `openspecpowers instructions apply --change <id>`
...
- **WHEN** user runs `openspecpowers instructions apply --change <id> --json`
- **THEN** the system outputs JSON with:
...
- **WHEN** user runs `openspecpowers artifact-experimental-setup --tool cursor`
...
- **WHEN** user runs `openspecpowers artifact-experimental-setup` without `--tool`
...
- **WHEN** user runs `openspecpowers artifact-experimental-setup --tool unknown-tool`
