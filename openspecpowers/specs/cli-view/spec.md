...
The `openspecpowers view` command provides a comprehensive dashboard view of the OpenSpecPowers project state, displaying specifications, changes, and progress metrics in a unified, visually appealing format to help developers quickly understand project status.
## Requirements
...
- **WHEN** user runs `openspecpowers view`
- **THEN** system displays a formatted dashboard with sections for summary, active changes, completed changes, and specifications

#### Scenario: No OpenSpecPowers directory

- **WHEN** user runs `openspecpowers view` in a directory without OpenSpecPowers
- **THEN** system displays error message "✗ No openspecpowers directory found"
