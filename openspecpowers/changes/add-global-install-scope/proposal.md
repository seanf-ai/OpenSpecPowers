...
OpenSpecPowers installation paths are currently inconsistent:
...
- Add install scope control in `openspecpowers config profile` interactive flow.
- Extend `openspecpowers config list` output with install scope source (`explicit`, `new-default`, `legacy-default`).
