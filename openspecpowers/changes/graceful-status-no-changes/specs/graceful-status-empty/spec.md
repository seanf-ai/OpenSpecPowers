...
- **WHEN** user runs `openspecpowers status` without `--change` and no change directories exist under `openspecpowers/changes/`
- **THEN** the CLI prints `No active changes. Create one with: openspecpowers new change <name>` to stdout and exits with code 0

#### Scenario: No changes exist, JSON mode
- **WHEN** user runs `openspecpowers status --json` without `--change` and no change directories exist
...
- **WHEN** user runs `openspecpowers status` without `--change` and one or more change directories exist
...
- **WHEN** user runs `openspecpowers status --change non-existent`
- **THEN** the CLI throws an error with message `Change 'non-existent' not found`

#### Scenario: Other commands unaffected
- **WHEN** user runs `openspecpowers show` or `openspecpowers instructions` without `--change` and no changes exist
