...
- **WHEN** user runs `openspecpowers update`
...
- **WHEN** user runs `openspecpowers update`
- **AND** a tool has generated OpenSpecPowers command files
- **AND** that tool has no OpenSpecPowers skill files (commands-only delivery)
...
- **THEN** the system SHALL display: "New in this version: /opsp:propose (combines new + ff). Try 'openspecpowers config profile core' for the streamlined 4-workflow experience."
...
- **WHEN** user runs `openspecpowers update` multiple times
...
- **WHEN** user runs `openspecpowers update` non-interactively (e.g., in CI)
...
- **THEN** the system SHALL display: "Detected new tool: Windsurf. Run 'openspecpowers init' to add it."
...
- **THEN** the system SHALL display one consolidated message listing all detected tools, for example: "Detected new tools: GitHub Copilot, Windsurf. Run 'openspecpowers init' to add them."
...
### Requirement: Update requires an OpenSpecPowers project
The update command SHALL only run inside an initialized OpenSpecPowers project.

#### Scenario: Update outside a project
- **WHEN** user runs `openspecpowers update`
- **AND** no `openspecpowers/` directory exists in the current working directory
- **THEN** the system SHALL display: "No OpenSpecPowers project found. Run 'openspecpowers init' to set up."
...
- **WHEN** user runs `openspecpowers update`
- **AND** project has workflows not in current profile (e.g., user switched from custom to core or deselected workflows via `openspecpowers config profile`)
