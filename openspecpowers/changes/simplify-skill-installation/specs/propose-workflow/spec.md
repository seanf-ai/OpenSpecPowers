...
The propose workflow SHALL combine change creation and artifact generation into a single command, reducing friction for new users while teaching them the OpenSpecPowers workflow through embedded guidance.
...
- **WHEN** user invokes `/opsp:propose "add user authentication"`
- **THEN** the system SHALL create a change directory with kebab-case name
- **THEN** the system SHALL create `.openspecpowers.yaml` in the change directory (via `openspecpowers new change`)
...
- **WHEN** user invokes `/opsp:propose` with a name that already exists
...
- **WHEN** user invokes `/opsp:propose "feature name"`
- **THEN** the result SHALL be functionally equivalent to invoking `/opsp:new "feature-name"` followed by `/opsp:ff feature-name`
