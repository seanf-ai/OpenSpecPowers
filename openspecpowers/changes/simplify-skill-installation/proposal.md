...
```text
0:00  $ openspecpowers init
      ✓ Done. Run /opsp:propose "your idea"

0:15  /opsp:propose "add user authentication"
...
Power users can customize via `openspecpowers config profile`.
...
`/opsp:propose` should naturally onboard users by explaining what it's doing:
...
When ready to implement, run /opsp:apply
```
...
Stored in existing global config (`~/.config/openspecpowers/config.json`). Not prompted during init.
...
openspecpowers config profile core     # preset shortcut (core workflows, preserves delivery)
```
...
**Existing users keep their current setup.** When `openspecpowers update` runs on a project with existing workflows and no `profile` in global config, it performs a one-time migration:
...
- `openspecpowers init` on a **new** project (no existing workflows) uses global config, defaulting to `core`
...
- Migration message mentions `propose` and suggests `openspecpowers config profile core` to opt in
- After migration, users can opt into `core` profile via `openspecpowers config profile core`
...
```json
// ~/.config/openspecpowers/config.json (extends existing)
{
  "telemetry": { ... },          // existing
...
| custom | user-defined | Pick exactly what you need via `openspecpowers config profile` |
