...
- **WHEN** a user runs `openspecpowers change graph`
...
- **WHEN** a user runs `openspecpowers change next`
...
- **WHEN** a user runs `openspecpowers change split <change-id>`
...
- **WHEN** a user runs `openspecpowers change split <change-id>` for a parent whose generated child directories already exist
