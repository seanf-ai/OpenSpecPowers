...
OpenSpecPowers needs usage analytics to understand adoption and inform product decisions. PostHog provides a privacy-conscious analytics platform suitable for open source projects.
...
- Full `openspecpowers telemetry` command - Can add later if users request
...
```typescript
{
  ...
  properties: {
    ...
    version: '1.2.3'      // OpenSpecPowers version
  }
}
```
...
```typescript
// ~/.config/openspecpowers/config.json
{
  "telemetry": {
