...
Acceptance:
- `openspecpowers show` exists and prints a helpful hint in non-interactive contexts when no args.
...
- [x] Create `ShowCommand` with:
  ...
  - Non-interactive path when `!itemName`:
    - Print hint with examples:
      - `openspecpowers show <item>`
      - `openspecpowers change show`
      - `openspecpowers spec show`
    - Exit with code 1.
...
Acceptance:
- `openspecpowers show <change-id> --json --deltas-only` matches `openspecpowers change show <id> --json --deltas-only` output.
- `openspecpowers show <spec-id> --json --requirements` matches `openspecpowers spec show <id> --json --requirements` output.
...
- [x] `src/commands/change.ts` → extend `show(changeName?: string, options?: { json?: boolean; requirementsOnly?: boolean; deltasOnly?: boolean; noInteractive?: boolean })`:
  ...
  - Non-interactive fallback: keep current behavior (print available IDs + `openspecpowers change list` hint, set `process.exitCode = 1`).
...
Acceptance:
- `openspecpowers change show` in non-interactive prints list hint and exits non-zero.
- `openspecpowers spec show` in non-interactive prints missing-arg error and exits non-zero.
...
- [x] Top-level `show` hint (non-interactive no-arg):
  - Lines include: `openspecpowers show <item>`, `openspecpowers change show`, `openspecpowers spec show`, and "Or run in an interactive terminal.".
...
- [x] `test/commands/change.interactive-show.test.ts` (non-interactive fallback)
  - Ensure `openspecpowers change show` without args prints available IDs + list hint and non-zero exit.
- [x] `test/commands/spec.interactive-show.test.ts` (non-interactive fallback)
  - Ensure `openspecpowers spec show` without args prints missing-arg error and non-zero exit.
...
- [x] Update `openspecpowers/README.md` usage examples to include the new `show` command with type detection and flags.
