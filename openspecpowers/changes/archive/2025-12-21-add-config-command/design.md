...
The `global-config` spec defines how OpenSpecPowers reads/writes `config.json`, but users currently must edit it by hand. This command provides a CLI interface to that config.
...
**Examples:**
```bash
openspecpowers config get featureFlags              # Returns object
openspecpowers config get featureFlags.experimental # Returns nested value
openspecpowers config set featureFlags.newFlag true
```
...
**Rationale:**
- Raw output enables piping: `VAR=$(openspecpowers config get key)`
