...
- **THEN** create only the openspecpowers directory structure and config.yaml
...
**Migration**: Skills in `.<tool>/skills/` provide all OpenSpecPowers workflow instructions. No manual file needed.
...
**Reason**: Old `/openspecpowers:*` slash commands are replaced by `/opsp:*` commands with richer functionality.

**Migration**: Use `/opsp:new`, `/opsp:continue`, `/opsp:apply` instead of `/openspecpowers:proposal`, `/openspecpowers:apply`, `/openspecpowers:archive`.
