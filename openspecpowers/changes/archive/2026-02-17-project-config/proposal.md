...
Add `openspecpowers/config.yaml` support for project-level configuration. This enables teams to customize OpenSpecPowers behavior without forking schemas, by providing context and rules that are injected into artifact generation.

## Motivation

Currently, customizing OpenSpecPowers requires forking entire schemas:
- Must copy all files even to add one rule
- Lose updates when openspecpowers upgrades
...
OpenSpecPowers customization follows two distinct paths:
...
│   + add rules                                                   │
│                                                                 │
│   openspecpowers/config.yaml            openspecpowers/schemas/my-flow/     │
│                                                                 │
│   ✓ Simple                        ✓ Full control                │
...
- Commands run on changes without `.openspecpowers.yaml` metadata
...
3. **Avoids premature API surface** - No standalone `openspecpowers config init` until feature graduates
...
```
Today (Experimental):
  openspecpowers artifact-experimental-setup
    → prompts for config creation
    → creates .claude/skills/
    → creates openspecpowers/config.yaml

Future (When graduating):
  openspecpowers init
    → prompts for config creation
    → creates openspecpowers/ directory
    → creates openspecpowers/config.yaml

  + standalone commands:
    openspecpowers config init
    openspecpowers config validate
    openspecpowers config set <key> <value>
```

**Why optional?**

Config is **additive**, not required:
- OpenSpecPowers works without config (uses defaults)
...
- Update `openspecpowers new change` to use config's schema as default
...
- Integration with standard `openspecpowers init` (will add when experimental graduates)
- Standalone `openspecpowers config init` command (may add in future change)
- `openspecpowers config validate` command (may add in future change)
...
✓ Created .claude/skills/openspecpowers-explore/SKILL.md
✓ Created .claude/skills/openspecpowers-new-change/SKILL.md
✓ Created .claude/skills/openspecpowers-continue-change/SKILL.md
✓ Created .claude/skills/openspecpowers-apply-change/SKILL.md
✓ Created .claude/skills/openspecpowers-ff-change/SKILL.md
✓ Created .claude/skills/openspecpowers-sync-specs/SKILL.md
✓ Created .claude/skills/openspecpowers-archive-change/SKILL.md

✓ Created .claude/commands/opsp/explore.md
✓ Created .claude/commands/opsp/new.md
✓ Created .claude/commands/opsp/continue.md
✓ Created .claude/commands/opsp/apply.md
✓ Created .claude/commands/opsp/ff.md
✓ Created .claude/commands/opsp/sync.md
✓ Created .claude/commands/opsp/archive.md

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

📋 Project Configuration (Optional)

Configure project defaults for OpenSpecPowers workflows.

? Create openspecpowers/config.yaml? (Y/n) Y
...
✓ Created openspecpowers/config.yaml

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

🎉 Setup Complete!

📖 Config created at: openspecpowers/config.yaml
...
To share with team:
  git add openspecpowers/config.yaml .claude/
  git commit -m "Setup OpenSpecPowers experimental workflow with project config"
...
cat > openspecpowers/config.yaml << 'EOF'
schema: spec-driven
...
/opsp:new my-feature --schema spec-driven
...
/opsp:new my-feature --schema tdd
...
openspecpowers instructions proposal --change my-feature
...
openspecpowers instructions specs --change my-feature
...
```bash
openspecpowers instructions design --change my-feature
...
```bash
# Commit config
git add openspecpowers/config.yaml
...
Always at `./openspecpowers/config.yaml` relative to project root. No XDG/global config for simplicity.
...
2. .openspecpowers.yaml in change directory   # Change-specific binding
3. openspecpowers/config.yaml schema field    # Project default (NEW)
...
6. **Create and validate config:**
   ...
   - Write to `openspecpowers/config.yaml` using YAML serializer
...
When `openspecpowers/config.yaml` already exists:
...
ℹ️  openspecpowers/config.yaml already exists. Skipping config creation.

   To update config, edit openspecpowers/config.yaml manually or:
   1. Delete openspecpowers/config.yaml
   2. Run openspecpowers artifact-experimental-setup again
...
try {
  ...
  if (configResult.createConfig) {
    ...
    console.log('✓ Created openspecpowers/config.yaml');
  }
} catch (error) {
...
1. **@inquirer/prompts** (recommended)
   ...
   - Already used in OpenSpecPowers (if not, lightweight addition)
...
- Verify change's `.openspecpowers.yaml` overrides config
...
- Test all four levels of schema resolution:
  ...
  2. Change metadata `.openspecpowers.yaml`
...
- Existing changes with `.openspecpowers.yaml` metadata aren't affected by config
