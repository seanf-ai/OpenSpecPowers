...
OpenSpecPowers currently has a fixed schema resolution order:
1. `--schema` CLI flag
2. `.openspecpowers.yaml` in change directory
...
The proposal introduces `openspecpowers/config.yaml` as a lightweight customization layer that sits between preset schemas and full forking. It allows teams to:
...
**Key stakeholders:**
- OpenSpecPowers users who need light customization without forking
- Teams sharing workflow conventions via committed config

## Goals / Non-Goals

**Goals:**
- Load and parse `openspecpowers/config.yaml` using Zod schema
...
- Config management commands (`openspecpowers config init`) - manual creation for now
...
- Consistent with `.openspecpowers.yaml` used in changes
...
### 2. Config Location: Project Root vs openspecpowers/ Directory

**Decision:** `./openspecpowers/config.yaml` (inside openspecpowers directory)

**Rationale:**
- Co-located with `openspecpowers/schemas/` (project-local-schemas)
- Keeps project root clean
- Natural namespace for OpenSpecPowers configuration
- Mirrors structure used by other tools (e.g., `.github/`)

**Alternatives considered:**
- `./openspecpowers.config.yaml` in root: Pollutes root, less clear ownership
...
3. **`openspecpowers/config.yaml` schema field** (NEW - project default)
...
**Rationale:**
- Syntax errors in config shouldn't break all of OpenSpecPowers
- User can fix config incrementally
- Commands remain usable during config development

**Warning message:**
```
⚠️  Failed to parse openspecpowers/config.yaml: [error details]
 ...
 * - Helps understand OpenSpecPowers's data interfaces at a glance
 * - Single source of truth for type and validation
 * - Consistent with other OpenSpecPowers schemas
 */
...
/**
 * Read and parse openspecpowers/config.yaml from project root.
...
export function readProjectConfig(): ProjectConfig | null {
  ...
  let configPath = path.join(projectRoot, 'openspecpowers', 'config.yaml');
  if (!existsSync(configPath)) {
    configPath = path.join(projectRoot, 'openspecpowers', 'config.yml');
    if (!existsSync(configPath)) {
  ...
  try {
    ...
    if (!raw || typeof raw !== 'object') {
      console.warn(`⚠️  openspecpowers/config.yaml is not a valid YAML object`);
      return null;
    }
    ...
    console.warn(`⚠️  Failed to parse openspecpowers/config.yaml:`, error);
    return null;
  }
}
...
export function suggestSchemas(
  ...
  let message = `❌ Schema '${invalidSchemaName}' not found in openspecpowers/config.yaml\n\n`;
  ...
  message += `\nFix: Edit openspecpowers/config.yaml and change 'schema: ${invalidSchemaName}' to a valid schema name`;
...
```typescript
// Example: "openspecpowers instructions proposal --change my-feature"
...
export function readProjectConfig(): ProjectConfig | null {
  const projectRoot = findProjectRoot();
  const configPath = path.join(projectRoot, 'openspecpowers', 'config.yaml');
...
┌──────────────────────────────────────────────────────────────┐
│                                                              │
│  User runs: openspecpowers instructions proposal --change foo     │
│                                                              │
└────────────────────────────┬─────────────────────────────────┘
                             │
                             ▼
...
│                                                              │
│  1. Check CLI flag ✗                                         │
│  2. Check .openspecpowers.yaml ✗                                   │
│  3. Check openspecpowers/config.yaml ✓ → "spec-driven"             │
│                                                              │
└────────────────────────────┬─────────────────────────────────┘
                             │
                             ▼
...
│  2. Read openspecpowers/config.yaml                                │
...
**[Risk]** YAML parsing errors break OpenSpecPowers commands
...
→ **Mitigation:** Document common artifact IDs, show in `openspecpowers status` output
