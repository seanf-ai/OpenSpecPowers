...
The `artifact-experimental-setup` command generates skill files and opsp slash commands for AI coding assistants. Currently it hardcodes paths to `.claude/skills` and `.claude/commands/opsp`.

The existing `AI_TOOLS` array in `config.ts` lists 22 AI tools but lacks path information. There's also an existing `SlashCommandConfigurator` system for the old workflow commands, but it's tightly coupled to the old 3 commands (proposal, apply, archive) and can't be easily extended for the 9 opsp commands.

Each AI tool has:
- Different skill directory conventions (`.claude/skills/`, `.cursor/skills/`, etc.)
- Different command file paths (`.claude/commands/opsp/`, `.cursor/commands/`, etc.)
...
interface CommandContent {
  ...
  name: string;         // e.g., 'OpenSpecPowers Explore'
  description: string;  // e.g., 'Enter explore mode...'
  category: string;     // e.g., 'OpenSpecPowers'
  tags: string[];       // e.g., ['openspecpowers', 'explore']
