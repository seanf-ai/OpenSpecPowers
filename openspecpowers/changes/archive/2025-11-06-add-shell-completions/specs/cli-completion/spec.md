...
- **WHEN** user executes `openspecpowers completion zsh`
...
- **THEN** remove `~/.oh-my-zsh/custom/completions/_openspecpowers` if Oh My Zsh is detected
...
- **WHEN** `openspecpowers completion install` cannot detect current shell or detects non-Zsh shell
- **THEN** display error: "Could not detect Zsh. Please specify explicitly: openspecpowers completion install zsh"
- **AND** exit with code 1
...
- **AND** allow redirection to files: `openspecpowers completion zsh > /path/to/_openspecpowers`
