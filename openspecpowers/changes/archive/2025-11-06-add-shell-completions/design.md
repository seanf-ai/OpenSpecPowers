...
**OpenSpecPowers Integration:**
```bash
# After installing: openspecpowers completion install bash
openspecpowers val<TAB>           # Completes to "openspecpowers validate"
openspecpowers validate <TAB><TAB>  # Shows: --all --changes --specs --strict --json [change-ids] [spec-ids]
openspecpowers show add-<TAB><TAB>  # Shows all changes starting with "add-"
```
...
**OpenSpecPowers Integration:**
```zsh
# After installing: openspecpowers completion install zsh
openspecpowers val<TAB>    # Shows menu with "validate" and "view" highlighted
openspecpowers show <TAB>  # Shows menu with all change IDs and spec IDs, categorized
```
...
**OpenSpecPowers Integration:**
```fish
# After installing: openspecpowers completion install fish
openspecpowers val       # Gray suggestion shows "validate" immediately
openspecpowers show a    # Real-time suggestions for changes starting with "a"
openspecpowers <TAB>     # Shows all commands with descriptions in paged menu
```
...
**OpenSpecPowers Integration:**
```powershell
# After installing: openspecpowers completion install powershell
openspecpowers val<TAB>       # Cycles: validate → view → validate
openspecpowers show <TAB>     # Cycles through change IDs one by one
openspecpowers <Ctrl+Space>   # Shows IntelliSense menu with all commands
```
...
const COMMAND_REGISTRY: CommandDefinition[] = [
  {
    name: 'init',
    description: 'Initialize OpenSpecPowers in your project',
    flags: [
...
class CompletionProvider {
  ...
  isOpenSpecPowersProject(): boolean {
    // Check for openspecpowers/ directory
  }
}
```
...
- Graceful degradation when outside OpenSpecPowers projects
...
│       ├── command-registry.ts    # Single source of truth for OpenSpecPowers commands
...
if (isOhMyZshInstalled()) {
  // Install to ~/.oh-my-zsh/custom/completions/_openspecpowers
  // Automatically loaded by Oh My Zsh
} else {
  // Install to ~/.zsh/completions/_openspecpowers
...
2. **Not in OpenSpecPowers project** - Skip dynamic completions, only offer static commands
...
3. **Manual Testing**
   ...
   - Tab completion behavior in OpenSpecPowers projects
