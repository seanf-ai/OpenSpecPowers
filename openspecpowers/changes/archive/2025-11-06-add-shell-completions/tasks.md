...
- [x] Create `src/core/completions/command-registry.ts` with `COMMAND_REGISTRY` constant defining all OpenSpecPowers commands, flags, and metadata
...
- [x] Implement installation to `~/.oh-my-zsh/custom/completions/_openspecpowers` for Oh My Zsh
- [x] Implement fallback installation to `~/.zsh/completions/_openspecpowers` with `fpath` updates
...
- [x] Implement dynamic completion helper functions in Zsh generator (`_openspecpowers_complete_changes`, `_openspecpowers_complete_specs`, `_openspecpowers_complete_items`)
...
- [ ] Test dynamic change/spec ID completion in real OpenSpecPowers projects
- [ ] Verify completion cache behavior (2-second TTL)
- [ ] Test behavior outside OpenSpecPowers projects (should skip dynamic completions)
- [x] Update `openspecpowers --help` output to include completion command (automatically done via Commander)
