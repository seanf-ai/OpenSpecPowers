...
- Full replacement for OpenSpecPowers-managed files only (e.g., `openspecpowers/README.md`)
...
- Use existing template utilities
  - `readmeTemplate` from `src/core/templates/readme-template.ts` for `openspecpowers/README.md`
  - `TemplateManager.getClaudeTemplate()` for `CLAUDE.md`
- Directory name is fixed to `openspecpowers` (from `OPENSPEC_DIR_NAME`)

### File Operations
- Use async utilities for consistency
  - `FileSystemUtils.writeFile` for `openspecpowers/README.md`
...
export class UpdateCommand {
  async execute(projectPath: string): Promise<void> {
    const openspecpowersDirName = OPENSPEC_DIR_NAME;
    const openspecpowersPath = path.join(projectPath, openspecpowersDirName);

    // 1. Check openspecpowers directory exists
    if (!await FileSystemUtils.directoryExists(openspecpowersPath)) {
      throw new Error(`No OpenSpecPowers directory found. Run 'openspecpowers init' first.`);
    }

    // 2. Update README.md (full replacement)
    const readmePath = path.join(openspecpowersPath, 'README.md');
    ...
    console.log('Updated OpenSpecPowers instructions');
  }
}
```
...
- Full overwrite only for OpenSpecPowers-managed files
- Marker-managed updates for user-owned files

## Error Handling

Only handle critical errors:
- Missing `openspecpowers` directory → throw error handled by CLI to present a friendly message
- File write failures → let errors bubble up to CLI

## Testing Strategy

Manual smoke tests are sufficient initially:
1. Run `openspecpowers init` in a test project
2. Modify both files (including custom content around markers in `CLAUDE.md`)
3. Run `openspecpowers update`
4. Verify `openspecpowers/README.md` fully replaced; `CLAUDE.md` OpenSpecPowers block updated without altering user content outside markers
5. Run the command twice to verify idempotency and no duplicate markers
6. Test with missing `openspecpowers` directory (expect failure)