# Update OpenSpecPowers Agent Instructions

## Why

The current OpenSpecPowers agent instructions need updates to follow best practices for AI assistant instructions (brevity, clarity, removing ambiguity), ensure CLI commands are current with the actual implementation, and properly document the three-stage workflow pattern that agents should follow.
...
- **Complete command documentation** with all current functionality:
  - `openspecpowers init [path]` - Initialize OpenSpecPowers in a project
  - `openspecpowers list` - List all active changes (default)
  - `openspecpowers list --specs` - List all specifications
  - `openspecpowers show [item]` - Display change or spec with auto-detection
  - `openspecpowers show` - Interactive mode for selection
  - `openspecpowers diff [change]` - Show spec differences for a change
  - `openspecpowers validate [item]` - Validate changes or specs
  - `openspecpowers archive [change]` - Archive completed change after deployment
  - `openspecpowers update [path]` - Update OpenSpecPowers instruction files
...
- **Remove deprecated command references** (noun-first patterns like `openspecpowers change show`)
- **Add concrete examples** for each command variation
- **Document debugging commands**:
  - `openspecpowers show [change] --json --deltas-only` for inspecting deltas
  - `openspecpowers validate [change] --strict` for comprehensive validation
...
- **Delta detection debugging**:
  - Use `openspecpowers show [change] --json --deltas-only` to inspect parsed deltas
...
- **Spec discovery workflow** - Always check existing specs before creating new ones:
  - Use `openspecpowers list --specs` to see all current specs
...
- Affected documentation:
  - `openspecpowers/README.md` - Main OpenSpecPowers instructions
  - CLAUDE.md files generated by `openspecpowers init` command

Note: This is a tooling/infrastructure change that doesn't require spec updates. When archiving, use `openspecpowers archive update-agent-instructions --skip-specs`.