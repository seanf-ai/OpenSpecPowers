...
|--------|---------|---------|
| **Task Progress** | Track implementation work | `openspecpowers view`, `openspecpowers list` |
| **Artifact Progress** | Track planning/spec work | `openspecpowers status`, `openspecpowers next` |
...
async function validateChangeExists(changeName: string, projectRoot: string): Promise<string> {
  const changePath = path.join(projectRoot, 'openspecpowers', 'changes', changeName);
  ...
  if (!fs.existsSync(changePath) || !fs.statSync(changePath).isDirectory()) {
    ...
    const entries = await fs.promises.readdir(
      path.join(projectRoot, 'openspecpowers', 'changes'),
      { withFileTypes: true }
    );
    ...
    if (available.length === 0) {
      throw new Error('No changes found. Create one with: openspecpowers new change <name>');
    }
...
```bash
# Before
$ openspecpowers new change foo
$ openspecpowers status --change foo
