...
Result: `openspecpowers new change foo && openspecpowers view` shows `foo` as "Completed" when it has no content.
...
Result: `openspecpowers status --change foo` says "not found" even though the directory exists.
...
|---------|--------|-------|
| `openspecpowers view` | Empty = "Completed" | Empty = "Draft" |
| `openspecpowers status --change X` | Requires proposal.md | Works on any directory |
| `openspecpowers validate X` | Requires proposal.md | Unchanged (still requires it) |

## Breaking Changes

### Minimal Breaking Change

1. **`openspecpowers view` output**: Empty changes move from "Completed" section to new "Draft" section

### Non-Breaking

- Active/Completed semantics unchanged (still task-based)
- `getActiveChangeIds()` unchanged
- `openspecpowers validate` unchanged
...
|---------|-----------------------------|-----------------------------|----------------|
| `openspecpowers view` | No (reads dirs directly) | Yes → Draft section | **Yes** |
| `openspecpowers list` | No (reads dirs directly) | Yes (shows "No tasks") | No |
| `openspecpowers status/next/instructions` | Yes | Yes | **Yes** |
| `openspecpowers validate` | Yes | No (can't validate empty) | No |
| `openspecpowers show` | Yes | No (nothing to show) | No |
| Tab completions | Yes | Future enhancement | No |

## Success Criteria

1. `openspecpowers new change foo && openspecpowers view` shows `foo` in "Draft" section
2. `openspecpowers new change foo && openspecpowers status --change foo` works
