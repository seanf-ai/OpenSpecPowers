...
OpenSpecPowers has a complete skill and slash command generation system. Skills are defined in `src/core/templates/skill-templates.ts` as functions that return `SkillTemplate` objects (for Agent Skills) and `CommandTemplate` objects (for slash commands). These are registered in `src/core/shared/skill-generation.ts` and generated during `openspecpowers init` and `openspecpowers update`.
...
**Goals:**
- Add `/opsp:onboard` skill that teaches the OpenSpecPowers workflow through guided practice
