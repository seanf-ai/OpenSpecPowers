...
The metadata file (`.openspecpowers.yaml`) will be validated with this Zod schema:
...
/**
 * Schema for per-change metadata stored in .openspecpowers.yaml
 */
...
**Location:** `openspecpowers/changes/<name>/.openspecpowers.yaml`
...
- `openspecpowers.json` - YAML matches existing schema files
...
const METADATA_FILENAME = '.openspecpowers.yaml';
...
2. **`.openspecpowers.yaml` metadata** (persisted choice)
...
No migration needed:
- Existing changes without `.openspecpowers.yaml` continue to work (use default)
- New changes created with `openspecpowers new change --schema X` get metadata file

## Open Questions

- Should `openspecpowers new change` prompt for schema interactively if not specified? (Leaning no - default is fine)
