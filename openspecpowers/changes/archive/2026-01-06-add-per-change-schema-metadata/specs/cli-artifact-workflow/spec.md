...
The system SHALL store and validate per-change metadata in `.openspecpowers.yaml` files using a Zod schema.

#### Scenario: Metadata file created with new change

- **WHEN** user runs `openspecpowers new change add-feature --schema tdd`
- **THEN** the system creates `.openspecpowers.yaml` in the change directory
- **AND** the file contains `schema: tdd` and `created: <YYYY-MM-DD>`

#### Scenario: Metadata validated on read

- **WHEN** the system reads `.openspecpowers.yaml`
...
- **WHEN** `.openspecpowers.yaml` contains invalid YAML or missing required fields
- **THEN** the system displays a Zod validation error with details

#### Scenario: Missing metadata file

- **WHEN** a change directory has no `.openspecpowers.yaml` file
...
- **AND** creates `.openspecpowers.yaml` with `schema: spec-driven` (default)

#### Scenario: Create change with schema

- **WHEN** user runs `openspecpowers new change add-feature --schema tdd`
- **THEN** the system creates `openspecpowers/changes/add-feature/` directory
- **AND** creates `.openspecpowers.yaml` with `schema: tdd`

#### Scenario: Invalid schema on create

- **WHEN** user runs `openspecpowers new change add-feature --schema unknown`
...
- **WHEN** user runs `openspecpowers status --change <id>` without `--schema`
- **AND** the change has `.openspecpowers.yaml` with `schema: tdd`
- **THEN** the system uses the `tdd` schema

#### Scenario: Explicit schema overrides metadata

- **WHEN** user runs `openspecpowers status --change <id> --schema spec-driven`
- **AND** the change has `.openspecpowers.yaml` with `schema: tdd`
...
- **AND** the change has no `.openspecpowers.yaml` file
