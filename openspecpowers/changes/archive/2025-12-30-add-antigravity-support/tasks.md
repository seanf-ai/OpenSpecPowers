...
- [x] 1.2 Generate `.agent/workflows/openspecpowers-proposal.md`, `openspecpowers-apply.md`, and `openspecpowers-archive.md` with YAML frontmatter restricted to a single `description` field for each stage and wrap the body in OpenSpecPowers markers.
...
- [x] 2.1 Detect existing Antigravity workflow files during `openspecpowers update` and refresh only the managed body, skipping creation when files are missing.
