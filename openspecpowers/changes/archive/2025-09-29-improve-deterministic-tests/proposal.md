...
state via `process.cwd()` and `openspecpowers/changes`. This makes outcomes depend on
...
- Each suite that touches filesystem discovery creates a temporary directory:
  - `openspecpowers/changes/sample-change/proposal.md`
  - `openspecpowers/changes/sample-change/specs/sample/spec.md`
...
- Tests that previously depended on repo state now:
  ...
  - Do not read real `openspecpowers/changes` during execution.
