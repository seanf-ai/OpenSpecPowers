...
The `openspecpowers diff` command adds unnecessary complexity to the OpenSpecPowers CLI for several reasons:

1. **Redundant functionality**: The `openspecpowers show` command already provides comprehensive visualization of changes through structured JSON output and markdown rendering
...
Remove the `openspecpowers diff` command entirely and guide users to more appropriate alternatives:

1. **For viewing change content**: Use `openspecpowers show <change-name>` which provides:
...
- **Better alignment**: Focuses on the core OpenSpecPowers workflow without redundant features

## Implementation

### Files to Remove
- `/src/core/diff.ts` - The entire diff command implementation
- `/openspecpowers/specs/cli-diff/spec.md` - The diff command specification
...
- `/openspecpowers/README.md` - Remove diff command references
- Various documentation files mentioning `openspecpowers diff`

### Migration Guide for Users

Users currently using `openspecpowers diff` should transition to:

```bash
# Before
openspecpowers diff add-feature

# After - view the change proposal
openspecpowers show add-feature

# After - view only the deltas
openspecpowers show add-feature --json --deltas-only

# After - use git for file comparisons
git diff openspecpowers/specs openspecpowers/changes/add-feature/specs
```
