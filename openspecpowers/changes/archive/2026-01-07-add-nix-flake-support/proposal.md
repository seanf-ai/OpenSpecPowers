...
OpenSpecPowers users on NixOS or using the Nix package manager cannot easily install or run OpenSpecPowers without going through npm. Adding a Nix flake makes OpenSpecPowers a first-class citizen in the Nix ecosystem, enabling users to run `nix run github:seanf-ai/OpenSpecPowers -- init` or include OpenSpecPowers in their development environments declaratively.
...
### New Capabilities
- `nix-flake-support`: Nix flake configuration for building and running OpenSpecPowers
