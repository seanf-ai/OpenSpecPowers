# Design: Add /opsp:verify Skill

## Architecture Decision: Dynamic Generation via Setup Command

### Context

All existing opsp experimental skills (explore, new, continue, apply, ff, sync, archive) are dynamically generated when users run `openspecpowers artifact-experimental-setup`. They are not manually created files checked into the repository.
...
1. **Consistency**: All 7 existing opsp skills follow this pattern. Adding verify as the 8th skill should follow the same architecture.
...
3. **Distribution**: Users get the verify skill automatically when running `openspecpowers artifact-experimental-setup`, just like all other opsp skills. No special installation steps needed.
...
- Update help text to list `/opsp:verify`

#### 3. Generated Artifacts

When users run `openspecpowers artifact-experimental-setup`, the command creates:

- `.claude/skills/openspecpowers-verify-change/SKILL.md` - Agent Skills format
- `.claude/commands/opsp/verify.md` - Slash command format
...
Create `.claude/skills/openspecpowers-verify-change/SKILL.md` as a static file in the OpenSpecPowers repository.

**Rejected because:**
- Inconsistent with all other opsp skills
...
Add `openspecpowers setup-verify` as a separate command.
...
3. Help text mentions `/opsp:verify`
4. Running `openspecpowers artifact-experimental-setup` generates both skill and command files
