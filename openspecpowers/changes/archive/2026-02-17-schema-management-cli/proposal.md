...
Add a new `openspecpowers schema` command group with subcommands for creating, forking, validating, and inspecting schemas.

### Commands

1. **`openspecpowers schema init <name>`** - Interactive wizard to scaffold a new project schema
   ...
   - Optionally sets as project default in `openspecpowers/config.yaml`

2. **`openspecpowers schema fork <source> [name]`** - Copy an existing schema as a starting point
...
3. **`openspecpowers schema validate [name]`** - Validate schema structure and templates
...
4. **`openspecpowers schema which <name>`** - Show schema resolution path
