...
### 1. Command Structure: `openspecpowers schema <subcommand>`

Add a new command group following the existing pattern used by `openspecpowers config` and `openspecpowers completion`.
...
- Flat commands (`openspecpowers schema-init`, `openspecpowers schema-fork`): Rejected because it pollutes the top-level namespace and doesn't scale well.
- Extending existing commands (`openspecpowers init --schema`): Rejected because schema management is distinct from project initialization.
...
**Rationale:** Matches the UX established by `openspecpowers init` and `openspecpowers config reset`. Provides a guided experience while keeping the wizard lightweight.
...
The destination is always project-local: `openspecpowers/schemas/<name>/`
...
**Rationale:** Consistent with existing OpenSpecPowers commands and enables scripting/automation.
