...
OpenSpecPowers currently resolves schemas from two locations:
1. User override: `~/.local/share/openspecpowers/schemas/<name>/`
2. Package built-in: `<npm-package>/schemas/<name>/`

This change adds a third, highest-priority level: project-local schemas at `./openspecpowers/schemas/<name>/`.
...
- Schema management CLI commands (`openspecpowers schema copy/which/diff/reset`)
...
```
1. ./openspecpowers/schemas/<name>/              # Project-local (highest)
2. ~/.local/share/openspecpowers/schemas/<name>/ # User override
...
function getProjectSchemasDir(projectRoot: string): string {
  return path.join(projectRoot, 'openspecpowers', 'schemas');
}
```
...
A team could create `openspecpowers/schemas/spec-driven/` that shadows the built-in, causing confusion when someone expects default behavior.

**Mitigation:** The `openspecpowers schemas` command shows the source of each schema. Users can see `spec-driven (project)` vs `spec-driven (package)`.
