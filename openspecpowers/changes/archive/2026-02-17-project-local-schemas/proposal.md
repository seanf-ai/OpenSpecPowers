...
Add project-local schema resolution (`./openspecpowers/schemas/`) as the highest priority in the schema lookup chain. This enables teams to version control custom workflow schemas with their repository.

## Motivation

Currently, schema resolution is 2-level:
1. User override: `~/.local/share/openspecpowers/schemas/<name>/`
...
```
1. ./openspecpowers/schemas/<name>/                    # Project-local (NEW)
2. ~/.local/share/openspecpowers/schemas/<name>/       # User global (XDG)
...
- `openspecpowers/schemas/my-workflow/` → referenced as `my-workflow`
...
- Schema management CLI (`openspecpowers schema copy/which/diff/reset`) - future enhancement
...
mkdir -p openspecpowers/schemas/my-workflow/templates

# Define schema
cat > openspecpowers/schemas/my-workflow/schema.yaml << 'EOF'
name: my-workflow
version: 1
...
# Create templates
echo "# Research\n\n..." > openspecpowers/schemas/my-workflow/templates/research.md
# ... etc
```
...
```bash
# Via CLI flag
openspecpowers new change add-feature --schema my-workflow
openspecpowers status --change add-feature --schema my-workflow
...
```bash
# Commit to repo
git add openspecpowers/schemas/
git commit -m "Add custom workflow schema"
git push

# Team members get it automatically
git pull
openspecpowers status --change add-feature --schema my-workflow  # Just works
```
...
Use existing `findProjectRoot()` pattern or current working directory. The project-local schemas directory is always `./openspecpowers/schemas/` relative to project root.
