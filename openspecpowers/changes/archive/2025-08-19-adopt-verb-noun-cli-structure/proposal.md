...
- Deprecate noun-based top-level commands: `openspecpowers spec ...` and `openspecpowers change ...`.
...
- **List**
  - From: `openspecpowers change list`
  - To: `openspecpowers list --changes` (default), or `openspecpowers list --specs`

- **Show**
  - From: `openspecpowers spec show <spec-id>` / `openspecpowers change show <change-id>`
  - To: `openspecpowers show <item-id>` with auto-detect, use `--type spec|change` if ambiguous

- **Validate**
  - From: `openspecpowers spec validate <spec-id>` / `openspecpowers change validate <change-id>`
  - To: `openspecpowers validate <item-id> --type spec|change`, or bulk: `openspecpowers validate --specs` / `--changes` / `--all`

### Backward Compatibility

- Keep `openspecpowers spec` and `openspecpowers change` available with deprecation warnings for one release cycle.
...
- **Affected specs**:
  ...
  - `openspecpowers-conventions`: Add explicit requirement establishing verb–noun CLI design and deprecation guidance
- **Affected code**:
...
**CLI Design**
- From: Mixed model with nouns (`spec`, `change`) and some top-level verbs; `openspecpowers list` currently deprecated
- To: Verbs as primary: `openspecpowers list|show|validate|diff|archive`; nouns scoped via flags or item ids; noun commands deprecated
...
**Listing Behavior**
- From: `openspecpowers change list` (primary), `openspecpowers list` (deprecated)
- To: `openspecpowers list` as primary, defaulting to `--changes`; add `--specs` to list specs
...
- Document new usage in `openspecpowers/README.md` and CLI help.
