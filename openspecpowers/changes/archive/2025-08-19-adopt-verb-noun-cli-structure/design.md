...
2. Add `--specs`/`--changes` to `openspecpowers list`, with `--changes` as default.
3. Show deprecation warnings for `openspecpowers change list` and, more generally, for any `openspecpowers change ...` and `openspecpowers spec ...` subcommands.
...
## Out of Scope
JSON output parity for `openspecpowers list` across modes and `show --specs/--changes` discovery are follow-ups.
