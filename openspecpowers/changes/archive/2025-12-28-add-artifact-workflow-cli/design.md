...
```
openspecpowers status --change <id>
openspecpowers next --change <id>
openspecpowers instructions <artifact> --change <id>
openspecpowers templates [--schema <name>]
openspecpowers new change <name>
```
...
```bash
openspecpowers status --change add-auth   # explicit, works
openspecpowers status                      # error: missing --change
```
...
```bash
openspecpowers new change add-feature
```

**Rationale:**
- `openspecpowers new <name>` is ambiguous (new what?)
- `openspecpowers new change <name>` is clear and extensible
- Can add `openspecpowers new spec <name>` later if needed
