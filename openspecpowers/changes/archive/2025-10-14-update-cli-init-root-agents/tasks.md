## 1. Implementation
- [x] 1.1 Refactor `openspecpowers init` to always generate the root `AGENTS.md` stub (initial run and extend mode) via shared helper logic.
...
- [x] 2.2 Execute `openspecpowers validate update-cli-init-root-agents --strict`.
- [x] 2.3 Perform a manual smoke test: run `openspecpowers init` in a temp directory, confirm stub + grouped prompts, rerun in extend mode.
