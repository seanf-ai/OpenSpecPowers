...
The command SHALL configure AI coding assistants with OpenSpecPowers instructions based on user selection.

#### Scenario: Prompting for AI tool selection

- **WHEN** run
- **THEN** prompt user to select AI tools to configure:
  - Claude Code (✅ OpenSpecPowers custom slash commands available)
  - Cursor (✅ OpenSpecPowers custom slash commands available)
...
- **THEN** create or update `AGENTS.md` in the project root directory (not inside openspecpowers/)
...
See @openspecpowers/AGENTS.md for detailed conventions and guidelines.
<!-- OPENSPEC:END -->
```
...
- **THEN** create new file with OpenSpecPowers content wrapped in markers using the same template as CLAUDE.md
...
- **THEN** preserve all existing content
- **AND** ensure the OpenSpecPowers-managed block at the beginning of the file is refreshed without duplicating markers
...
- **AND** allow OpenSpecPowers to update its content without affecting user customizations
