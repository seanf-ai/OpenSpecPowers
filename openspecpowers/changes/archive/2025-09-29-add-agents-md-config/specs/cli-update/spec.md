...
- **THEN** the command SHALL:
  ...
  - Replace `openspecpowers/AGENTS.md` with the latest template (complete replacement)
...
- **AND** create or update the root-level `AGENTS.md` using the OpenSpecPowers markers
- **AND** update only the OpenSpecPowers-managed blocks in **existing** AI tool files using markers
- **AND** use the default directory name `openspecpowers`
...
- **AND** display the message: "Updated OpenSpecPowers instructions"
