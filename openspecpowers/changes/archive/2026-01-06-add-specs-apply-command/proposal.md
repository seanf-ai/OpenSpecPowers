...
Spec application is currently bundled with archive - users must run `openspecpowers archive` to apply delta specs to main specs. This couples two distinct concerns (applying specs vs. archiving the change) and forces users to wait until they're "done" to see main specs updated. Users want the flexibility to sync specs earlier in the workflow while iterating.

## What Changes

- Add `/opsp:sync` skill that syncs delta specs to main specs as a standalone action
...
**Workflow becomes:**
```
/opsp:new → /opsp:continue → /opsp:apply → archive
                                  │
                                  └── /opsp:sync (optional, anytime)
```

## Capabilities

### New Capabilities
- `specs-sync-skill`: Skill template for `/opsp:sync` command that reconciles main specs with delta specs
...
- **Skills**: New `openspecpowers-sync-specs` skill in `skill-templates.ts`
