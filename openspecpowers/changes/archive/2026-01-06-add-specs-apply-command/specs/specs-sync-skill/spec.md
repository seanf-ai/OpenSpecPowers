...
The system SHALL provide an `/opsp:sync` skill that syncs delta specs from a change to the main specs.

#### Scenario: Sync delta specs to main specs
- **WHEN** agent executes `/opsp:sync` with a change name
- **THEN** the agent reads delta specs from `openspecpowers/changes/<name>/specs/`
- **AND** reads corresponding main specs from `openspecpowers/specs/`
- **AND** reconciles main specs to match what the deltas describe

#### Scenario: Idempotent operation
- **WHEN** agent executes `/opsp:sync` multiple times on the same change
- **THEN** the result is the same as running it once
- **AND** no duplicate requirements are created

#### Scenario: Change selection prompt
- **WHEN** agent executes `/opsp:sync` without specifying a change
...
- **THEN** create new main spec file at `openspecpowers/specs/<capability>/spec.md`
