...
Currently, explore references `/opsp:new` and `/opsp:ff` which are being replaced with `/opsp:propose`. But beyond just updating references, there are deeper UX questions about how explore should work.
...
2. **Where should exploration files live?**
   - `openspecpowers/explorations/<name>.md`?
   - `openspecpowers/changes/<change>/explorations/`?
   - Somewhere else?
...
6. **What happens before a change proposal exists?**
   ...
   - When ready, user runs `/opsp:propose`
...
- Just update references to `/opsp:propose`
...
- Saves to `openspecpowers/explorations/<name>.md`
...
- `openspecpowers/changes/simplify-skill-installation/` - current change updating core workflows
