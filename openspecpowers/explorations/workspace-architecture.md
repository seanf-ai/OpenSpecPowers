...
```
openspecpowers profile set core|extended
openspecpowers profile install <workflow>
openspecpowers profile uninstall <workflow>
openspecpowers profile list
openspecpowers profile show
openspecpowers config set delivery skills|commands|both
openspecpowers config get delivery
...
**After (simplified):**
```
openspecpowers config profile          # interactive picker (delivery + workflows)
openspecpowers config profile core     # preset shortcut
openspecpowers config profile extended # preset shortcut
```
...
- **Global** = user's defaults (`~/.config/openspecpowers/`)
- **Project** = repo-level config (`.openspecpowers/` or committed to repo)
...
Keep it simple:
1. Global profile as default for `openspecpowers init`
2. `openspecpowers init` applies current profile to project
...
### Current OpenSpecPowers Architecture

OpenSpecPowers currently assumes:
- One `openspecpowers/` per repo, always at root
...
## Part 4: Three Models for OpenSpecPowers
...
├── services/
│   └── billing/
│       └── openspecpowers/specs/billing/spec.md
├── clients/
│   ├── web/
│   │   └── openspecpowers/specs/checkout/spec.md
│   ├── ios/
│   │   └── openspecpowers/specs/checkout/spec.md
│   └── android/
│       └── openspecpowers/specs/checkout/spec.md
└── openspecpowers/           # Root-level for cross-cutting
    ├── specs/
...
- Works for multi-repo too (each repo has its own `openspecpowers/`)

**Cons:**
- Cross-cutting specs are awkward (where do they go?)
- Changes that span multiple `openspecpowers/` directories = ???
- Need a "workspace" concept to aggregate
- Multiple `openspecpowers/` roots to manage

### Model D: Hybrid (Model B Inside Each Project + Model C Across Projects)

Use one `openspecpowers/` root per project, but allow nested specs within that root for clear ownership and shared contracts.
...
~/work/
├── billing-service/
│   └── openspecpowers/specs/billing/
├── web-client/
│   └── openspecpowers/specs/checkout/
├── ios-client/
│   └── openspecpowers/specs/checkout/
└── contracts/                    # Dedicated repo for shared specs?
    └── openspecpowers/specs/
        └── checkout-contract/
```
...
2. **Where do cross-repo changes live?**
   ...
   - In `~/.config/openspecpowers/workspaces/my-platform/changes/`?
...
> **A workspace is a collection of OpenSpecPowers roots that can be operated on together.**

```yaml
# ~/.config/openspecpowers/workspaces.yaml (or similar)
workspaces:
  my-platform:
    roots:
...
- `openspecpowers/specs/checkout/spec.md` is the shared kernel
- Platform specs nest under it

**Option B: Dedicated area**
- `openspecpowers/specs/_shared/checkout-contract/spec.md`
- Or `openspecpowers/specs/_contracts/checkout/spec.md`
...
| Concept | Definition |
|---------|------------|
| **Project** | Single OpenSpecPowers root (one `openspecpowers/` directory) |
...
### 4. Does OpenSpecPowers need to understand dependencies?

If `checkout-web` depends on `checkout-contract`:
- Should OpenSpecPowers know this relationship?
...
**Ambitious:**
> OpenSpecPowers automatically understands your repo structure, detects cross-cutting specs, and helps you create changes that flow to the right places.

**Simpler:**
> You organize specs however you want. OpenSpecPowers just works.
...
2. `openspecpowers init` improvements
3. No workspace tracking yet
...
### What is a spec in OpenSpecPowers?

For OpenSpecPowers, a spec should be treated as a **verifiable behavior contract at a boundary**:
...
OpenSpecPowers is often agent-authored from human exploration. To make that reliable:
...
2. `openspecpowers/specs/openspecpowers-conventions/spec.md` for normative spec conventions
3. `openspecpowers/specs/docs-agent-instructions/spec.md` for agent-instruction authoring rules
...
|----------|--------|-------|
| Profile UX | Decided | `openspecpowers config profile` with presets |
