...
OpenSpecPowers provides three levels of customization:
...
The `openspecpowers/config.yaml` file is the easiest way to customize OpenSpecPowers for your team. It lets you:
...
```bash
# Without config
openspecpowers new change my-feature --schema spec-driven

# With config - schema is automatic
openspecpowers new change my-feature
```
...
When OpenSpecPowers needs a schema, it checks in this order:

1. CLI flag: `--schema <name>`
2. Change metadata (`.openspecpowers.yaml` in the change folder)
...
When project config isn't enough, create your own schema with a completely custom workflow. Custom schemas live in your project's `openspecpowers/schemas/` directory and are version-controlled with your code.
...
This copies the entire `spec-driven` schema to `openspecpowers/schemas/my-workflow/` where you can edit it freely.

**What you get:**

```text
openspecpowers/schemas/my-workflow/
...
```yaml
# openspecpowers/schemas/my-workflow/schema.yaml
name: my-workflow
version: 1
...
```bash
# Specify on command
openspecpowers new change feature --schema my-workflow
...
Schema: my-workflow
Source: project
Path: /path/to/project/openspecpowers/schemas/my-workflow
```

---

> **Note:** OpenSpecPowers also supports user-level schemas at `~/.local/share/openspecpowers/schemas/` for sharing across projects, but project-level schemas in `openspecpowers/schemas/` are recommended since they're version-controlled with your code.
...
```yaml
# openspecpowers/schemas/rapid/schema.yaml
name: rapid
version: 1
...
```bash
openspecpowers schema fork spec-driven with-review
```
