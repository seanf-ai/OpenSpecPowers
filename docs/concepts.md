...
This guide explains the core ideas behind OpenSpecPowers and how they fit together. For practical usage, see [Getting Started](getting-started.md) and [Workflows](workflows.md).

## Philosophy

OpenSpecPowers is built around four principles:
...
**Fluid not rigid.** Traditional spec systems lock you into phases: first you plan, then you implement, then you're done. OpenSpecPowers is more flexible — you can create artifacts in any order that makes sense for your work.

**Iterative not waterfall.** Requirements change. Understanding deepens. What seemed like a good approach at the start might not hold up after you see the codebase. OpenSpecPowers embraces this reality.

**Easy not complex.** Some spec frameworks require extensive setup, rigid formats, or heavyweight processes. OpenSpecPowers stays out of your way. Initialize in seconds, start working immediately, customize only if you need to.

**Brownfield-first.** Most software work isn't building from scratch — it's modifying existing systems. OpenSpecPowers's delta-based approach makes it easy to specify changes to existing behavior, not just describe new systems.

## The Big Picture

OpenSpecPowers organizes your work into two main areas:
...
OpenSpecPowers aims to avoid bureaucracy. Use the lightest level that still makes the change verifiable.
...
```
openspecpowers/changes/add-dark-mode/
├── proposal.md           # Why and what
├── design.md             # How (technical approach)
├── tasks.md              # Implementation checklist
├── .openspecpowers.yaml        # Change metadata (optional)
└── specs/                # Delta specs
    └── ui/
...
Delta specs are the key concept that makes OpenSpecPowers work for brownfield development. They describe **what's changing** rather than restating the entire spec.
...
```yaml
# openspecpowers/schemas/spec-driven/schema.yaml
name: spec-driven
artifacts:
  - id: proposal
...
openspecpowers schema fork spec-driven research-first
```

**Example custom schema:**

```yaml
# openspecpowers/schemas/research-first/schema.yaml
name: research-first
artifacts:
  - id: research
...
│                                                                              │
│   ┌────────────────┐                                                         │
│   │  1. START      │  /opsp:propose (core) or /opsp:new (expanded)          │
│   │     CHANGE     │                                                         │
│   └───────┬────────┘                                                         │
│           │                                                                  │
│           ▼                                                                  │
│   ┌────────────────┐                                                         │
│   │  2. CREATE     │  /opsp:ff or /opsp:continue (expanded workflow)         │
...
| **Source of truth** | The `openspecpowers/specs/` directory, containing the current agreed-upon behavior |
