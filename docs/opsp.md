...
> Feedback welcome via [GitHub Issues](https://github.com/seanf-ai/OpenSpecPowers/issues).

## What Is It?

OPSP is now the standard workflow for OpenSpecPowers.

It's a **fluid, iterative workflow** for OpenSpecPowers changes. No more rigid phases — just actions you can take anytime.

## Why This Exists

The legacy OpenSpecPowers workflow works, but it's **locked down**:
...
**OPSP opens it up.** Now anyone can:
...
- **OpenSpecPowers contributors** — experiment with new approaches without releases

We're all still learning what works best. OPSP lets us learn together.
...
```bash
# Make sure you have openspecpowers installed — skills are automatically generated
openspecpowers init
```

This creates skills in `.claude/skills/` (or equivalent) that AI coding assistants auto-detect.

By default, OpenSpecPowers uses the `core` workflow profile (`propose`, `explore`, `apply`, `archive`). If you want the expanded workflow commands (`new`, `continue`, `ff`, `verify`, `sync`, `bulk-archive`, `onboard`), configure them with `openspecpowers config profile` and apply with `openspecpowers update`.

During setup, you'll be prompted to create a **project config** (`openspecpowers/config.yaml`). This is optional but recommended.
...
Config is created during `openspecpowers init`, or manually:

```yaml
# openspecpowers/config.yaml
schema: spec-driven
...
2. Change metadata (`.openspecpowers.yaml` in change directory)
...
- Run `openspecpowers schemas --json` to see artifact IDs for each schema

**Config not being applied:**
- Ensure file is at `openspecpowers/config.yaml` (not `.yml`)
...
|---------|--------------|
| `/opsp:propose` | Create a change and generate planning artifacts in one step (default quick path) |
| `/opsp:explore` | Think through ideas, investigate problems, clarify requirements |
| `/opsp:new` | Start a new change scaffold (expanded workflow) |
| `/opsp:continue` | Create the next artifact (expanded workflow) |
| `/opsp:ff` | Fast-forward planning artifacts (expanded workflow) |
| `/opsp:apply` | Implement tasks, updating artifacts as needed |
| `/opsp:verify` | Validate implementation against artifacts (expanded workflow) |
| `/opsp:sync` | Sync delta specs to main (expanded workflow, optional) |
| `/opsp:archive` | Archive when done |
| `/opsp:bulk-archive` | Archive multiple completed changes (expanded workflow) |
| `/opsp:onboard` | Guided walkthrough of an end-to-end change (expanded workflow) |
...
/opsp:explore
```
Think through ideas, investigate problems, compare options. No structure required - just a thinking partner. When insights crystallize, transition to `/opsp:propose` (default) or `/opsp:new`/`/opsp:ff` (expanded).
...
```text
/opsp:new        # scaffold only
/opsp:continue   # create one artifact at a time
/opsp:ff         # create all planning artifacts at once
```
...
/opsp:apply
```
Works through tasks, checking them off as you go. If you're juggling multiple changes, you can run `/opsp:apply <name>`; otherwise it should infer from the conversation and prompt you to choose if it can't tell.

### Finish up
```
/opsp:archive   # Move to archive when done (prompts to sync specs if needed)
```
...
| | Legacy (`/openspecpowers:proposal`) | OPSP (`/opsp:*`) |
|---|---|---|
...
**The key insight:** work isn't linear. OPSP stops pretending it is.

## Architecture Deep Dive

This section explains how OPSP works under the hood and how it compares to the legacy workflow.
...
│         │                     │                     │                       │
│         ▼                     ▼                     ▼                       │
│   /openspecpowers:proposal   /openspecpowers:apply      /openspecpowers:archive              │
│                                                                             │
...
│                    │                                                        │
│                    ▼                                                        │
│   Generated Command Files (.claude/commands/openspecpowers/*.md)                  │
│                                                                             │
...
**OPSP** uses external schemas and a dependency graph engine:
...
│                    │                                                        │
│                    ▼                                                        │
│   Skill Files (.claude/skills/openspecpowers-*/SKILL.md)                          │
│                                                                             │
...
**OPSP** — agent queries for rich context:
...
```
  ...
  │  ┌────────────────────────────────────────────────────────────────────┐  │
  │  │  $ openspecpowers status --change "add-auth" --json                      │  │
  │  │                                                                    │  │
  │  │  {                                                                 │  │
  │  │    "artifacts": [                                                  │  │
  ...
  │  ┌────────────────────────────────────────────────────────────────────┐  │
  │  │  $ openspecpowers instructions specs --change "add-auth" --json          │  │
  │  │                                                                    │  │
  │  │  {                                                                 │  │
...
**OPSP** — natural iteration:

```
  /opsp:new ───► /opsp:continue ───► /opsp:apply ───► /opsp:archive
      │                │                  │
      │                │                  ├── "The design is wrong"
      │                │                  │
      │                │                  ▼
...
openspecpowers schema which my-workflow
```

Schemas are stored in `openspecpowers/schemas/` (project-local, version controlled) or `~/.local/share/openspecpowers/schemas/` (user global).

**Schema structure:**
```
openspecpowers/schemas/research-first/
├── schema.yaml
└── templates/
    ├── research.md
    ├── proposal.md
...
openspecpowers schema which --all
...
- Use `/opsp:explore` to think through an idea before committing to a change
- `/opsp:ff` when you know what you want, `/opsp:continue` when exploring
- During `/opsp:apply`, if something's wrong — fix the artifact, then continue
- Tasks track progress via checkboxes in `tasks.md`
- Check status anytime: `openspecpowers status --change "name"`

## Feedback

This is rough. That's intentional — we're learning what works.

Found a bug or have ideas? Open an issue on [GitHub](https://github.com/seanf-ai/OpenSpecPowers/issues).
