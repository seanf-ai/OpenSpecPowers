...
This guide helps you transition from the legacy OpenSpecPowers workflow to OPSP. The migration is designed to be smooth—your existing work is preserved, and the new system offers more flexibility.

## What's Changing?

OPSP replaces the old phase-locked workflow with a fluid, action-based approach. Here's the key shift:

| Aspect | Legacy | OPSP |
|--------|--------|------|
| **Commands** | `/openspecpowers:proposal`, `/openspecpowers:apply`, `/openspecpowers:archive` | Default: `/opsp:propose`, `/opsp:apply`, `/opsp:archive` (expanded workflow commands optional) |
| **Workflow** | Create all artifacts at once | Create incrementally or all at once—your choice |
| **Going back** | Awkward phase gates | Natural—update any artifact anytime |
| **Customization** | Fixed structure | Schema-driven, fully hackable |
| **Configuration** | `CLAUDE.md` with markers + `project.md` | Clean config in `openspecpowers/config.yaml` |

**The philosophy change:** Work isn't linear. OPSP stops pretending it is.
...
- **Active changes in `openspecpowers/changes/`** — Completely preserved. You can continue them with OPSP commands.
- **Archived changes** — Untouched. Your history remains intact.
- **Main specs in `openspecpowers/specs/`** — Untouched. These are your source of truth.
- **Your content in CLAUDE.md, AGENTS.md, etc.** — Preserved. Only the OpenSpecPowers marker blocks are removed; everything you wrote stays.

### What Gets Removed

Only OpenSpecPowers-managed files that are being replaced:

| What | Why |
|------|-----|
| Legacy slash command directories/files | Replaced by the new skills system |
| `openspecpowers/AGENTS.md` | Obsolete workflow trigger |
| OpenSpecPowers markers in `CLAUDE.md`, `AGENTS.md`, etc. | No longer needed |

**Legacy command locations by tool** (examples—your tool may vary):

- Claude Code: `.claude/commands/openspecpowers/`
- Cursor: `.cursor/commands/openspecpowers-*.md`
- Windsurf: `.windsurf/workflows/openspecpowers-*.md`
- Cline: `.clinerules/workflows/openspecpowers-*.md`
- Roo: `.roo/commands/openspecpowers-*.md`
- GitHub Copilot: `.github/prompts/openspecpowers-*.prompt.md` (IDE extensions only; not supported in Copilot CLI)
...
The removal list may seem long, but these are all files that OpenSpecPowers originally created. Your own content is never deleted.

### What Needs Your Attention

One file requires manual migration:

**`openspecpowers/project.md`** — This file isn't deleted automatically because it may contain project context you've written. You'll need to:

1. Review its contents
2. Move useful context to `openspecpowers/config.yaml` (see guidance below)
...
The new `config.yaml` context is **actively injected into every OpenSpecPowers planning request**. This means your project conventions, tech stack, and rules are always present when the AI is creating artifacts. Higher reliability.
...
Both `openspecpowers init` and `openspecpowers update` detect legacy files and guide you through the same cleanup process. Use whichever fits your situation:
...
```
Upgrading to the new OpenSpecPowers

OpenSpecPowers now uses agent skills, the emerging standard across coding
...
Files to update
OpenSpecPowers markers will be removed, your content preserved:
  • CLAUDE.md
  • AGENTS.md
...
Needs your attention
  • openspecpowers/project.md
    ...
    The new openspecpowers/config.yaml has a "context:" section for planning
    context. This is included in every OpenSpecPowers request and works more
...
2. OpenSpecPowers markers are stripped from `CLAUDE.md`, `AGENTS.md`, etc. (your content stays)
3. `openspecpowers/AGENTS.md` is deleted
4. New skills are installed in `.claude/skills/`
5. `openspecpowers/config.yaml` is created with a default schema

### Using `openspecpowers update`
...
```bash
openspecpowers init --force --tools claude
```
...
The old `openspecpowers/project.md` was a freeform markdown file for project context. The new `openspecpowers/config.yaml` is structured and—critically—**injected into every planning request** so your conventions are always present when the AI works.
...
```
I'm migrating from OpenSpecPowers's old project.md to the new config.yaml format.
...
| `/opsp:explore` | Think through ideas with no structure |
| `/opsp:apply` | Implement tasks from tasks.md |
| `/opsp:archive` | Finalize and archive the change |
...
| `/opsp:continue` | Create the next artifact (one at a time) |
| `/opsp:ff` | Fast-forward—create planning artifacts at once |
| `/opsp:verify` | Validate implementation matches specs |
| `/opsp:sync` | Preview/spec-merge without archiving |
| `/opsp:bulk-archive` | Archive multiple changes at once |
| `/opsp:onboard` | Guided end-to-end onboarding workflow |

Enable expanded commands with `openspecpowers config profile`, then run `openspecpowers update`.

### Command Mapping from Legacy

| Legacy | OPSP Equivalent |
|--------|-----------------|
| `/openspecpowers:proposal` | `/opsp:propose` (default) or `/opsp:new` then `/opsp:ff` (expanded) |
...
When you run `/opsp:continue`, it checks what's ready and offers the next artifact. You can also create multiple ready artifacts in any order.
...
OPSP uses the emerging **skills** standard:
...
Your in-progress changes work seamlessly with OPSP commands.

**Have an active change from the legacy workflow?**

```
/opsp:apply add-my-feature
```

OPSP reads the existing artifacts and continues from where you left off.

**Want to add more artifacts to an existing change?**

```
/opsp:continue add-my-feature
```
...
```bash
openspecpowers status --change add-my-feature
```
...
When determining which schema to use, OPSP checks in order:

1. **CLI flag**: `--schema <name>` (highest priority)
2. **Change metadata**: `.openspecpowers.yaml` in the change directory
3. **Project config**: `openspecpowers/config.yaml`
...
```bash
openspecpowers schema init my-workflow
```
...
```bash
openspecpowers schemas --json
```

### Config not being applied

1. Ensure the file is at `openspecpowers/config.yaml` (not `.yml`)
...
├── .claude/
│   └── skills/                   # NEW: OPSP skills
│       ├── openspecpowers-propose/     # default core profile
│       ├── openspecpowers-explore/
│       ├── openspecpowers-apply-change/
│       └── ...                   # expanded profile adds new/continue/ff/etc.
├── CLAUDE.md                     # OpenSpecPowers markers removed, your content preserved
└── AGENTS.md                     # OpenSpecPowers markers removed, your content preserved
```

### What's Gone

- `.claude/commands/openspecpowers/` — replaced by `.claude/skills/`
- `openspecpowers/AGENTS.md` — obsolete
- `openspecpowers/project.md` — migrate to `config.yaml`, then delete
- OpenSpecPowers marker blocks in `CLAUDE.md`, `AGENTS.md`, etc.

### Command Cheatsheet

```text
/opsp:propose      Start quickly (default core profile)
/opsp:apply        Implement tasks
/opsp:archive      Finish and archive

# Expanded workflow (if enabled):
/opsp:new          Scaffold a change
/opsp:continue     Create next artifact
/opsp:ff           Create planning artifacts
```
...
- **GitHub Issues**: [github.com/seanf-ai/OpenSpecPowers/issues](https://github.com/seanf-ai/OpenSpecPowers/issues)
- **Discussions**: [github.com/seanf-ai/OpenSpecPowers/discussions](https://github.com/seanf-ai/OpenSpecPowers/discussions)
- **Documentation**: [docs/opsp.md](opsp.md) for the full OPSP reference
