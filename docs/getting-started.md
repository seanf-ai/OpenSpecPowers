...
This guide explains how OpenSpecPowers works after you've installed and initialized it. For installation instructions, see the [main README](../README.md#quick-start).

## How It Works

OpenSpecPowers helps you and your AI coding assistant agree on what to build before any code is written.
...
```text
/opsp:new ──► /opsp:ff or /opsp:continue ──► /opsp:apply ──► /opsp:verify ──► /opsp:archive
```

The default global profile is `core`, which includes `propose`, `explore`, `apply`, and `archive`. You can enable the expanded workflow commands with `openspecpowers config profile` and then `openspecpowers update`.

## What OpenSpecPowers Creates

After running `openspecpowers init`, your project has this structure:
...
Delta specs are the key concept in OpenSpecPowers. They show what's changing relative to your current specs.
...
The change folder moves to `openspecpowers/changes/archive/` for audit history.
...
If you've enabled the expanded workflow profile, you can also do this as two steps: `/opsp:new` then `/opsp:ff` (or `/opsp:continue` incrementally).
...
AI:  Archiving add-dark-mode...
     ✓ Merged specs into openspecpowers/specs/ui/spec.md
...
- [Customization](customization.md) - Make OpenSpecPowers work your way
