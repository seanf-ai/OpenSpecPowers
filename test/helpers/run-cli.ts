...
function resolveBuildCommand(): { command: string; args: string[] } {
  const userPm = process.env.OPENSPECPOWERS_TEST_PM?.trim();
  if (userPm === 'npm') {
  ...
  // Prefer pnpm to match project scripts, but fall back to npm in minimal environments.
...
export async function ensureCliBuilt() {
  ...
  if (!buildPromise) {
    ...
    buildPromise = runCommand(buildCmd.command, buildCmd.args).catch((error) => {
      if (buildCmd.command === 'pnpm') {
        // Fall back automatically when pnpm is unavailable in CI/sandbox environments.
