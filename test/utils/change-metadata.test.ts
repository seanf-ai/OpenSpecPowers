...
describe('writeChangeMetadata', () => {
  ...
  beforeEach(async () => {
    ...
    changeDir = path.join(testDir, 'openspecpowers', 'changes', 'test-change');
  ...
  it('should write valid YAML metadata file', async () => {
    ...
    const metaPath = path.join(changeDir, '.openspecpowers.yaml');
...
describe('resolveSchemaForChange', () => {
  ...
  it('should use project config schema when no metadata exists', async () => {
    ...
    const configDir = path.join(testDir, 'openspecpowers');
