...
describe('top-level show command', () => {
  ...
  const changesDir = path.join(testDir, 'openspecpowers', 'changes');
  ...
  it('prints hint and non-zero exit when no args and non-interactive', () => {
    ...
    try {
      ...
      let err: any;
      try {
        execSync(`node ${openspecpowersBin} show`, { encoding: 'utf-8' });
      } catch (e) { err = e; }
      expect(err).toBeDefined();
      expect(err.status).not.toBe(0);
      const stderr = err.stderr.toString();
      expect(stderr).toContain('Nothing to show.');
      expect(stderr).toContain('openspecpowers show <item>');
      expect(stderr).toContain('openspecpowers change show');
      expect(stderr).toContain('openspecpowers spec show');
    } finally {
  ...
  it('auto-detects change id and supports --json', () => {
    ...
    try {
      process.chdir(testDir);
      const output = execSync(`node ${openspecpowersBin} show demo --json`, { encoding: 'utf-8' });
  ...
  it('auto-detects spec id and supports spec-only flags', () => {
    ...
    try {
      process.chdir(testDir);
      const output = execSync(`node ${openspecpowersBin} show auth --json --requirements`, { encoding: 'utf-8' });
      const json = JSON.parse(output);
  ...
  it('handles ambiguity and suggests --type', async () => {
    ...
    try {
      ...
      let err: any;
      try {
        execSync(`node ${openspecpowersBin} show foo`, { encoding: 'utf-8' });
      } catch (e) { err = e; }
      expect(err).toBeDefined();
      expect(err.status).not.toBe(0);
      const stderr = err.stderr.toString();
  ...
  it('prints nearest matches when not found', () => {
    ...
    try {
      ...
      let err: any;
      try {
        execSync(`node ${openspecpowersBin} show unknown-item`, { encoding: 'utf-8' });
      } catch (e) { err = e; }
      expect(err).toBeDefined();
      expect(err.status).not.toBe(0);
      const stderr = err.stderr.toString();
