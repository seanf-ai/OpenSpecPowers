...
async function runConfigCommand(args: string[]): Promise<void> {
  ...
  await program.parseAsync(['node', 'openspecpowers', 'config', ...args]);
}
...
describe('config profile interactive flow', () => {
  ...
  function setupDriftedProjectArtifacts(projectDir: string): void {
    fs.mkdirSync(path.join(projectDir, 'openspecpowers'), { recursive: true });
    const exploreSkillPath = path.join(projectDir, '.claude', 'skills', 'openspecpowers-explore', 'SKILL.md');
    fs.mkdirSync(path.dirname(exploreSkillPath), { recursive: true });
    fs.writeFileSync(exploreSkillPath, 'name: openspecpowers-explore\n', 'utf-8');
  }
  ...
  function setupSyncedCoreBothArtifacts(projectDir: string): void {
    ...
    for (const commandId of coreCommands) {
      const commandPath = path.join(projectDir, '.claude', 'commands', 'opsp', `${commandId}.md`);
  ...
  function addExtraSyncWorkflowArtifacts(projectDir: string): void {
    const syncSkillPath = path.join(projectDir, '.claude', 'skills', 'openspecpowers-sync-specs', 'SKILL.md');
    fs.mkdirSync(path.dirname(syncSkillPath), { recursive: true });
    fs.writeFileSync(syncSkillPath, 'name: openspecpowers-sync-specs\n', 'utf-8');

    const syncCommandPath = path.join(projectDir, '.claude', 'commands', 'opsp', 'sync.md');
  ...
  beforeEach(() => {
    ...
    tempDir = path.join(os.tmpdir(), `openspecpowers-config-profile-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
  ...
  it('selecting current values only should be a no-op and should not ask apply', async () => {
    ...
    fs.mkdirSync(path.join(tempDir, 'openspecpowers'), { recursive: true });
    select.mockResolvedValueOnce('delivery');
