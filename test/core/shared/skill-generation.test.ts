...
describe('skill-generation', () => {
  describe('getSkillTemplates', () => {
    ...
    it('should include all expected skills', () => {
      ...
      expect(dirNames).toContain('openspecpowers-explore');
      expect(dirNames).toContain('openspecpowers-new-change');
      expect(dirNames).toContain('openspecpowers-continue-change');
      expect(dirNames).toContain('openspecpowers-apply-change');
      expect(dirNames).toContain('openspecpowers-ff-change');
      expect(dirNames).toContain('openspecpowers-sync-specs');
      expect(dirNames).toContain('openspecpowers-archive-change');
      expect(dirNames).toContain('openspecpowers-bulk-archive-change');
      expect(dirNames).toContain('openspecpowers-verify-change');
      expect(dirNames).toContain('openspecpowers-onboard');
      expect(dirNames).toContain('openspecpowers-propose');
    });
    ...
    it('should return single template when filter has one workflow', () => {
      ...
      expect(filtered[0].dirName).toBe('openspecpowers-propose');
    });
  });
  ...
  describe('generateSkillContent', () => {
    ...
    it('should use default values for optional fields', () => {
      ...
      expect(content).toContain('license: MIT');
      expect(content).toContain('compatibility: Requires openspecpowers CLI.');
      expect(content).toContain('author: openspecpowers');
    ...
    it('should apply transformInstructions callback when provided', () => {
      const template = {
        ...
        instructions: 'Use /opsp:new to start and /opsp:apply to implement.',
      };

      const transformer = (text: string) => text.replace(/\/opsp:/g, '/opsp-');
      const content = generateSkillContent(template, '0.23.0', transformer);

      expect(content).toContain('/opsp-new');
      expect(content).toContain('/opsp-apply');
      expect(content).not.toContain('/opsp:new');
      expect(content).not.toContain('/opsp:apply');
    });
    ...
    it('should not transform instructions when callback is undefined', () => {
      const template = {
        ...
        instructions: 'Use /opsp:new to start.',
      };
