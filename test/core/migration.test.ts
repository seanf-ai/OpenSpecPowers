...
describe('migration', () => {
  ...
  beforeEach(async () => {
    projectDir = path.join(os.tmpdir(), `openspecpowers-migration-project-${randomUUID()}`);
    configHome = path.join(os.tmpdir(), `openspecpowers-migration-config-${randomUUID()}`);
  ...
  it('migrates to custom skills delivery when only managed skills are detected', async () => {
    await writeSkill(projectDir, 'openspecpowers-explore');
    await writeSkill(projectDir, 'openspecpowers-apply-change');
  ...
  it('ignores unknown custom skill and command files when scanning workflows', async () => {
    ...
    const customCommandPath = path.join(projectDir, '.claude', 'commands', 'opsp', 'my-custom.md');
