...
describe('ListCommand', () => {
  ...
  beforeEach(async () => {
    ...
    tempDir = path.join(os.tmpdir(), `openspecpowers-list-test-${Date.now()}`);
  ...
  describe('execute', () => {
    it('should handle missing openspecpowers/changes directory', async () => {
      const listCommand = new ListCommand();
      
      await expect(listCommand.execute(tempDir, 'changes')).rejects.toThrow(
        "No OpenSpecPowers changes directory found. Run 'openspecpowers init' first."
      );
    });

    it('should handle empty changes directory', async () => {
      const changesDir = path.join(tempDir, 'openspecpowers', 'changes');
