...
describe('FishInstaller', () => {
  ...
  beforeEach(async () => {
    testHomeDir = path.join(os.tmpdir(), `openspecpowers-fish-test-${randomUUID()}`);
  ...
  describe('getInstallationPath', () => {
    it('should return standard fish completions path', () => {
      ...
      expect(result).toBe(path.join(testHomeDir, '.config', 'fish', 'completions', 'openspecpowers.fish'));
    });
    ...
    it('should use homeDir from constructor', () => {
      ...
      expect(result).toBe(path.join(customHome, '.config', 'fish', 'completions', 'openspecpowers.fish'));
    });
  });
  ...
  describe('install', () => {
    const mockCompletionScript = `# Fish completion script for OpenSpecPowers CLI
function __fish_openspecpowers
    echo "test"
end

complete -c openspecpowers -a 'init' -d 'Initialize OpenSpecPowers'
`;
    ...
    it('should install completion script for the first time', async () => {
      ...
      expect(result.installedPath).toBe(path.join(testHomeDir, '.config', 'fish', 'completions', 'openspecpowers.fish'));
    ...
    it('should create parent directories if they do not exist', async () => {
      ...
      expect(result.success).toBe(true);
      const targetPath = path.join(testHomeDir, '.config', 'fish', 'completions', 'openspecpowers.fish');
    ...
    it('should update when content is different', async () => {
      ...
      const updatedScript = `# Fish completion script for OpenSpecPowers CLI
function __fish_openspecpowers_new
    echo "updated"
end

complete -c openspecpowers -a 'init' -d 'Initialize OpenSpecPowers'
complete -c openspecpowers -a 'validate' -d 'Validate specs'
`;
    ...
    it('should handle installation with paths containing spaces', async () => {
      const spacedHomeDir = path.join(os.tmpdir(), `openspecpowers fish test ${randomUUID()}`);
      ...
      expect(result.success).toBe(true);
      expect(result.installedPath).toContain('openspecpowers fish test');
...
complete -c openspecpowers -a 'init'
`;
