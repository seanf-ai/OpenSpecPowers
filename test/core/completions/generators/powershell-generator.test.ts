...
describe('PowerShellGenerator', () => {
	...
	describe('generate', () => {
		it('should generate valid PowerShell completion script with header', () => {
			...
			expect(script).toContain('# PowerShell completion script for OpenSpecPowers CLI');
			expect(script).toContain('$openspecpowersCompleter = {');
			expect(script).toContain('Register-ArgumentCompleter');
		});

		it('should register argument completer for openspecpowers command', () => {
			const commands: CommandDefinition[] = [
				{
					name: 'init',
					description: 'Initialize OpenSpecPowers',
			...
			expect(script).toContain('Register-ArgumentCompleter -CommandName openspecpowers');
			expect(script).toContain('-ScriptBlock $openspecpowersCompleter');
		});
		...
		it('should handle positional arguments for change-id', () => {
			...
			expect(script).toContain('Get-OpenSpecPowersChanges');
		});
		...
		it('should handle positional arguments for spec-id', () => {
			...
			expect(script).toContain('Get-OpenSpecPowersSpecs');
		});
		...
		it('should generate dynamic completion helper for changes', () => {
			...
			expect(script).toContain('function Get-OpenSpecPowersChanges');
			expect(script).toContain('openspecpowers __complete changes 2>$null');
		...
		it('should generate dynamic completion helper for specs', () => {
			...
			expect(script).toContain('function Get-OpenSpecPowersSpecs');
			expect(script).toContain('openspecpowers __complete specs 2>$null');
		});
