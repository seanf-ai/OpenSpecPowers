...
describe('UpdateCommand', () => {
  ...
  beforeEach(async () => {
    ...
    testDir = path.join(os.tmpdir(), `openspecpowers-test-${randomUUID()}`);
    await fs.mkdir(testDir, { recursive: true });

    // Create openspecpowers directory
    const openspecpowersDir = path.join(testDir, 'openspecpowers');
    await fs.mkdir(openspecpowersDir, { recursive: true });
  ...
  describe('basic validation', () => {
    it('should throw error if openspecpowers directory does not exist', async () => {
      // Remove openspecpowers directory
      await fs.rm(path.join(testDir, 'openspecpowers'), {
        recursive: true,
        force: true,
      });

      await expect(updateCommand.execute(testDir)).rejects.toThrow(
        "No OpenSpecPowers directory found. Run 'openspecpowers init' first."
      );
    });
  ...
  describe('skill updates', () => {
    it('should update skill files for configured Claude tool', async () => {
      ...
      const exploreSkillDir = path.join(skillsDir, 'openspecpowers-explore');
      await fs.mkdir(exploreSkillDir, { recursive: true });

      // Create an existing skill file
      const oldSkillContent = `---
name: openspecpowers-explore (old)
description: Old description
license: MIT
compatibility: Requires openspecpowers CLI.
metadata:
  author: openspecpowers
  version: "0.9"
---
...
`;
      ...
      expect(updatedSkill).toContain('name: openspecpowers-explore');
    ...
    it('should update core profile skill files when tool is configured', async () => {
      ...
      await fs.mkdir(path.join(skillsDir, 'openspecpowers-explore'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(skillsDir, 'openspecpowers-explore', 'SKILL.md'),
        'old content'
      );
  ...
  describe('command updates', () => {
    it('should update opsp commands for configured Claude tool', async () => {
      ...
      // Check opsp command files were created
    ...
    it('should update core profile opsp commands when tool is configured', async () => {
  ...
  describe('multi-tool support', () => {
    it('should update multiple configured tools', async () => {
      ...
      await fs.mkdir(path.join(claudeSkillsDir, 'openspecpowers-explore'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(claudeSkillsDir, 'openspecpowers-explore', 'SKILL.md'),
        'old'
      );

      // Set up Cursor
      const cursorSkillsDir = path.join(testDir, '.cursor', 'skills');
      await fs.mkdir(path.join(cursorSkillsDir, 'openspecpowers-explore'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(cursorSkillsDir, 'openspecpowers-explore', 'SKILL.md'),
        'old'
      );
      ...
      expect(claudeSkill).toContain('name: openspecpowers-explore');
      ...
      expect(cursorSkill).toContain('name: openspecpowers-explore');
    ...
    it('should update Qwen tool with correct command format', async () => {
      ...
      await fs.mkdir(path.join(qwenSkillsDir, 'openspecpowers-explore'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(qwenSkillsDir, 'openspecpowers-explore', 'SKILL.md'),
        'old'
      );

      await updateCommand.execute(testDir);

      // Check Qwen command format (TOML) - Qwen uses flat path structure: opsp-<id>.toml
    ...
    it('should update Windsurf tool with correct command format', async () => {
      ...
      await fs.mkdir(path.join(windsurfSkillsDir, 'openspecpowers-explore'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(windsurfSkillsDir, 'openspecpowers-explore', 'SKILL.md'),
        'old'
      );
  ...
  describe('skill content validation', () => {
    ...
    it('should include proper instructions in skill files', async () => {
      ...
      const skillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(skillsDir, 'openspecpowers-apply-change'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(skillsDir, 'openspecpowers-apply-change', 'SKILL.md'),
        'old'
      );
...
`
    ...
    it('should update all tools when force is used with mixed versions', async () => {
      ...
      const { version } = await import('../../package.json');
      const claudeSkillDir = path.join(testDir, '.claude', 'skills', 'openspecpowers-explore');
...
`
      ...
      const cursorSkillDir = path.join(testDir, '.cursor', 'skills', 'openspecpowers-explore');
...
`
  ...
  describe('version tracking', () => {
    ...
    it('should only update tools that need updating', async () => {
      ...
      const claudeSkillFile = path.join(testDir, '.claude', 'skills', 'openspecpowers-explore', 'SKILL.md');
  ...
  describe('legacy cleanup', () => {
    it('should detect and auto-cleanup legacy files with --force flag', async () => {
      ...
      // Create legacy CLAUDE.md with OpenSpecPowers markers
...
`;
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('Upgrading to the new OpenSpecPowers')
      );

      // Should show marker removal message (config files are never deleted, only have markers removed)
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('Removed OpenSpecPowers markers from CLAUDE.md')
      );
...
`;
    ...
    it('should cleanup legacy slash command directories with --force', async () => {
      ...
      const legacyCommandDir = path.join(testDir, '.claude', 'commands', 'openspecpowers');
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('Removed .claude/commands/openspecpowers/')
      );
    ...
    it('should cleanup legacy openspecpowers/AGENTS.md with --force', async () => {
      // Set up a configured tool
      const skillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(skillsDir, 'openspecpowers-explore'), {
        recursive: true,
      ...
      // Create legacy openspecpowers/AGENTS.md
      await fs.writeFile(
        path.join(testDir, 'openspecpowers', 'AGENTS.md'),
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('Removed openspecpowers/AGENTS.md')
      );
    ...
    it('should not show legacy cleanup messages when no legacy files exist', async () => {
      ...
      const hasLegacyMessage = calls.some(call =>
        call.includes('Upgrading to the new OpenSpecPowers')
      );
    ...
    it('should remove OpenSpecPowers marker block from mixed content files', async () => {
      // Set up a configured tool
      const skillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(skillsDir, 'openspecpowers-explore'), {
        recursive: true,
      ...
      // Create CLAUDE.md with mixed content (user content + OpenSpecPowers markers)
...
`;
  ...
  describe('legacy tool upgrade', () => {
    it('should upgrade legacy tools to new skills with --force', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('/opsp:new')
      );

      // Skills should be created
      const skillFile = path.join(testDir, '.claude', 'skills', 'openspecpowers-explore', 'SKILL.md');
    ...
    it('should upgrade multiple legacy tools with --force', async () => {
      ...
      await fs.writeFile(
        path.join(testDir, '.claude', 'commands', 'openspecpowers', 'proposal.md'),
        'content'
      );

      await fs.mkdir(path.join(testDir, '.cursor', 'commands'), { recursive: true });
      await fs.writeFile(
        path.join(testDir, '.cursor', 'commands', 'openspecpowers-proposal.md'),
        'content'
      );
      ...
      const claudeSkillFile = path.join(testDir, '.claude', 'skills', 'openspecpowers-explore', 'SKILL.md');
      const cursorSkillFile = path.join(testDir, '.cursor', 'skills', 'openspecpowers-explore', 'SKILL.md');
    ...
    it('should not upgrade legacy tools already configured', async () => {
      ...
      const skillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(skillsDir, 'openspecpowers-explore'), { recursive: true });
      await fs.writeFile(
        path.join(skillsDir, 'openspecpowers-explore', 'SKILL.md'),
        'existing skill'
      );
    ...
    it('should upgrade only unconfigured legacy tools when mixed', async () => {
      ...
      const claudeSkillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(claudeSkillsDir, 'openspecpowers-explore'), { recursive: true });
      await fs.writeFile(
        path.join(claudeSkillsDir, 'openspecpowers-explore', 'SKILL.md'),
        'existing skill'
      );
    ...
    it('should create only effective profile skills when upgrading legacy tools', async () => {
      ...
      const nonCoreSkill = path.join(skillsDir, 'openspecpowers-new-change', 'SKILL.md');
    ...
    it('should create commands when upgrading legacy tools', async () => {
      ...
      // New opsp commands should be created
      const commandsDir = path.join(testDir, '.claude', 'commands', 'opsp');
      const exploreCmd = path.join(commandsDir, 'explore.md');
      const exists = await FileSystemUtils.fileExists(exploreCmd);
      expect(exists).toBe(true);
    ...
    it('should not inject non-profile workflows when upgrading legacy tools', async () => {
      ...
      expect(await FileSystemUtils.fileExists(
        path.join(skillsDir, 'openspecpowers-propose', 'SKILL.md')
      )).toBe(false);
  ...
  describe('profile-aware updates', () => {
    it('should generate only profile workflows when custom profile is set', async () => {
      ...
      const skillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(skillsDir, 'openspecpowers-explore'), { recursive: true });
      await fs.writeFile(path.join(skillsDir, 'openspecpowers-explore', 'SKILL.md'), 'old');
      ...
      expect(await FileSystemUtils.fileExists(
        path.join(skillsDir, 'openspecpowers-new-change', 'SKILL.md')
      )).toBe(true);
...
`
    ...
    it('should remove workflows outside profile during update sync', async () => {
      ...
      await fs.mkdir(path.join(skillsDir, 'openspecpowers-new-change'), { recursive: true });
      await fs.writeFile(path.join(skillsDir, 'openspecpowers-new-change', 'SKILL.md'), 'old');
      const extraCommandFile = path.join(testDir, '.claude', 'commands', 'opsp', 'new.md');
  ...
  describe('new tool detection', () => {
    it('should detect new tool directories not currently configured', async () => {
      ...
      const claudeSkillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(claudeSkillsDir, 'openspecpowers-explore'), { recursive: true });
      await fs.writeFile(path.join(claudeSkillsDir, 'openspecpowers-explore', 'SKILL.md'), 'old');
      ...
      const hasNewToolMessage = calls.some(call =>
        call.includes("Detected new tool: Cursor. Run 'openspecpowers init' to add it.")
      );
    ...
    it('should consolidate multiple new tools into one message', async () => {
      ...
      expect(consolidatedCalls[0]).toContain("Run 'openspecpowers init' to add them.");
  ...
  describe('scanInstalledWorkflows', () => {
    it('should detect installed workflows across tools', async () => {
      ...
      const claudeSkillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(claudeSkillsDir, 'openspecpowers-explore'), { recursive: true });
      await fs.writeFile(path.join(claudeSkillsDir, 'openspecpowers-explore', 'SKILL.md'), 'content');
      await fs.mkdir(path.join(claudeSkillsDir, 'openspecpowers-apply-change'), { recursive: true });
      await fs.writeFile(path.join(claudeSkillsDir, 'openspecpowers-apply-change', 'SKILL.md'), 'content');
    ...
    it('should return union of workflows across multiple tools', async () => {
      ...
      // Cursor has apply
      const cursorSkillsDir = path.join(testDir, '.cursor', 'skills');
      await fs.mkdir(path.join(cursorSkillsDir, 'openspecpowers-apply-change'), { recursive: true });
      await fs.writeFile(path.join(cursorSkillsDir, 'openspecpowers-apply-change', 'SKILL.md'), 'content');
