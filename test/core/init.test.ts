...
describe('InitCommand', () => {
  ...
  beforeEach(async () => {
    testDir = path.join(os.tmpdir(), `openspecpowers-init-test-${Date.now()}`);
    await fs.mkdir(testDir, { recursive: true });
    originalEnv = { ...process.env };
    // Use a temp dir for global config to avoid reading real config
    configTempDir = path.join(os.tmpdir(), `openspecpowers-config-init-${Date.now()}`);
  ...
  describe('execute with --tools flag', () => {
    it('should create OpenSpecPowers directory structure', async () => {
      const initCommand = new InitCommand({ tools: 'claude', force: true });

      await initCommand.execute(testDir);

      const openspecpowersPath = path.join(testDir, 'openspecpowers');
      expect(await directoryExists(openspecpowersPath)).toBe(true);
      expect(await directoryExists(path.join(openspecpowersPath, 'specs'))).toBe(true);
      expect(await directoryExists(path.join(openspecpowersPath, 'changes'))).toBe(true);
      expect(await directoryExists(path.join(openspecpowersPath, 'changes', 'archive'))).toBe(true);
    });
    ...
    it('should create config.yaml with default schema', async () => {
      ...
      const configPath = path.join(testDir, 'openspecpowers', 'config.yaml');
    ...
    it('should create skills in Cursor skills directory', async () => {
      ...
      const skillFile = path.join(testDir, '.cursor', 'skills', 'openspecpowers-explore', 'SKILL.md');
      expect(await fileExists(skillFile)).toBe(true);
    });
    ...
    it('should create skills in Windsurf skills directory', async () => {
      ...
      const skillFile = path.join(testDir, '.windsurf', 'skills', 'openspecpowers-explore', 'SKILL.md');
      expect(await fileExists(skillFile)).toBe(true);
    });
    ...
    it('should create skills for multiple tools at once', async () => {
      ...
      const claudeSkill = path.join(testDir, '.claude', 'skills', 'openspecpowers-explore', 'SKILL.md');
      const cursorSkill = path.join(testDir, '.cursor', 'skills', 'openspecpowers-explore', 'SKILL.md');
    ...
    it('should select all tools with --tools all option', async () => {
      ...
      const claudeSkill = path.join(testDir, '.claude', 'skills', 'openspecpowers-explore', 'SKILL.md');
      const cursorSkill = path.join(testDir, '.cursor', 'skills', 'openspecpowers-explore', 'SKILL.md');
      const windsurfSkill = path.join(testDir, '.windsurf', 'skills', 'openspecpowers-explore', 'SKILL.md');
    ...
    it('should skip tool configuration with --tools none option', async () => {
      ...
      // Should create OpenSpecPowers structure but no skills
      const openspecpowersPath = path.join(testDir, 'openspecpowers');
      expect(await directoryExists(openspecpowersPath)).toBe(true);
    ...
    it('should not create config.yaml if it already exists', async () => {
      ...
      const configPath = path.join(openspecpowersDir, 'config.yaml');
    ...
    it('should handle non-existent target directory', async () => {
      ...
      const openspecpowersPath = path.join(newDir, 'openspecpowers');
      expect(await directoryExists(openspecpowersPath)).toBe(true);
    });
  ...
  describe('skill content validation', () => {
    it('should generate valid SKILL.md with YAML frontmatter', async () => {
      ...
      expect(content).toContain('name: openspecpowers-explore');
      expect(content).toContain('description:');
    ...
    it('should include propose skill instructions', async () => {
      ...
      const skillFile = path.join(testDir, '.claude', 'skills', 'openspecpowers-propose', 'SKILL.md');
      const content = await fs.readFile(skillFile, 'utf-8');

      expect(content).toContain('name: openspecpowers-propose');
    });
    ...
    it('should include apply-change skill instructions', async () => {
      ...
      const skillFile = path.join(testDir, '.claude', 'skills', 'openspecpowers-apply-change', 'SKILL.md');
      const content = await fs.readFile(skillFile, 'utf-8');

      expect(content).toContain('name: openspecpowers-apply-change');
    });
  ...
  describe('command generation', () => {
    it('should generate Claude Code commands with correct format', async () => {
      ...
      const cmdFile = path.join(testDir, '.claude', 'commands', 'opsp', 'explore.md');
    ...
    it('should generate Cursor commands with correct format', async () => {
      ...
      const cmdFile = path.join(testDir, '.cursor', 'commands', 'opsp-explore.md');
      expect(await fileExists(cmdFile)).toBe(true);
      ...
      vi.spyOn(fs, 'writeFile').mockImplementation(
        async (filePath: any, ...args: any[]) => {
          if (
            ...
            filePath.includes('.openspecpowers-test-')
          ) {
  ...
  describe('tool-specific adapters', () => {
    it('should generate Gemini CLI commands as TOML files', async () => {
      ...
      const cmdFile = path.join(testDir, '.gemini', 'commands', 'opsp', 'explore.toml');
      expect(await fileExists(cmdFile)).toBe(true);
    ...
    it('should generate Windsurf commands', async () => {
      ...
      const cmdFile = path.join(testDir, '.windsurf', 'workflows', 'opsp-explore.md');
      expect(await fileExists(cmdFile)).toBe(true);
    });
    ...
    it('should generate Continue prompt files', async () => {
      ...
      const cmdFile = path.join(testDir, '.continue', 'prompts', 'opsp-explore.prompt');
      expect(await fileExists(cmdFile)).toBe(true);

      const content = await fs.readFile(cmdFile, 'utf-8');
      expect(content).toContain('name: opsp-explore');
    ...
    it('should generate Cline workflow files', async () => {
      ...
      const cmdFile = path.join(testDir, '.clinerules', 'workflows', 'opsp-explore.md');
      expect(await fileExists(cmdFile)).toBe(true);
    });
    ...
    it('should generate GitHub Copilot prompt files', async () => {
      ...
      const cmdFile = path.join(testDir, '.github', 'prompts', 'opsp-explore.prompt.md');
      expect(await fileExists(cmdFile)).toBe(true);
    });
  });
});
...
describe('InitCommand - profile and detection features', () => {
  ...
  beforeEach(async () => {
    testDir = path.join(os.tmpdir(), `openspecpowers-init-profile-test-${Date.now()}`);
    await fs.mkdir(testDir, { recursive: true });
    originalEnv = { ...process.env };
    // Use a temp dir for global config to avoid polluting real config
    configTempDir = path.join(os.tmpdir(), `openspecpowers-config-test-${Date.now()}`);
    await fs.mkdir(configTempDir, { recursive: true });
    process.env.XDG_CONFIG_HOME = configTempDir;
    vi.spyOn(console, 'log').mockImplementation(() => {});
    confirmMock.mockReset();
  ...
  it('should use --profile flag to override global config', async () => {
    ...
    const proposeSkill = path.join(testDir, '.claude', 'skills', 'openspecpowers-propose', 'SKILL.md');
    expect(await fileExists(proposeSkill)).toBe(true);

    // Non-core skills (from the custom profile) should NOT be created
    const newChangeSkill = path.join(testDir, '.claude', 'skills', 'openspecpowers-new-change', 'SKILL.md');
  ...
  it('should auto-cleanup legacy artifacts in non-interactive mode without --force', async () => {
    ...
    await fs.writeFile(path.join(legacyDir, 'opsp-propose.md'), 'legacy content');
    ...
    expect(await fileExists(path.join(legacyDir, 'opsp-propose.md'))).toBe(false);
  ...
  it('should preselect configured tools but not directory-detected tools in extend mode', async () => {
    // Simulate existing OpenSpecPowers project (extend mode).
    await fs.mkdir(path.join(testDir, 'openspecpowers'), { recursive: true });

    // Configured with OpenSpecPowers
    ...
    // Directory detected only (not configured with OpenSpecPowers)
  ...
  it('should preselect detected tools for first-time interactive setup', async () => {
    // First-time init: no openspecpowers/ directory and no configured OpenSpecPowers skills.
    await fs.mkdir(path.join(testDir, '.github'), { recursive: true });
  ...
  it('should respect custom profile from global config', async () => {
    ...
    const exploreSkill = path.join(testDir, '.claude', 'skills', 'openspecpowers-explore', 'SKILL.md');
    const newChangeSkill = path.join(testDir, '.claude', 'skills', 'openspecpowers-new-change', 'SKILL.md');
  ...
  it('should migrate commands-only extend mode to custom profile without injecting propose', async () => {
    await fs.mkdir(path.join(testDir, 'openspecpowers'), { recursive: true });
    await fs.mkdir(path.join(testDir, '.claude', 'commands', 'opsp'), { recursive: true });
    await fs.writeFile(path.join(testDir, '.claude', 'commands', 'opsp', 'explore.md'), '# explore\n');
    ...
    const exploreCommand = path.join(testDir, '.claude', 'commands', 'opsp', 'explore.md');
    const proposeCommand = path.join(testDir, '.claude', 'commands', 'opsp', 'propose.md');
